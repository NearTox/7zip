...
bool ReadNamesFromListFile2(
    CFSTR fileName, UStringVector& strings, UINT codePage, DWORD& lastError) {
  lastError = 0;
  ...
  if (fileSize >= ((UInt32)1 << 31) - 32) return false;
  UString u;
  if (codePage == MY__CP_UTF16 || codePage == MY__CP_UTF16BE) {
    if ((fileSize & 1) != 0) return false;
    ...
    if (processed != fileSize) return false;
    file.Close();
    ...
    if (s.Len() != processed) return false;

    // #ifdef CP_UTF8
    if (codePage == CP_UTF8) {
      if (!ConvertUTF8ToUnicode(s, u)) return false;
    } else
      // #endif
  ...
  for (; i < u.Len(); i++) {
    wchar_t c = u[i];
    if (c == kGoodBOM || c == kBadBOM) return false;
    if (c == '\n' || c == 0xD) {
      AddName(strings, s);
      s.Empty();
    } else
