...
bool CStdOutStream::Close() throw() {
  ...
  if (fclose(_stream) != 0) return false;
  _stream = 0;
  _streamIsOpen = false;
  return true;
}

bool CStdOutStream::Flush() throw() { return (fflush(_stream) == 0); }

CStdOutStream& endl(CStdOutStream& outStream) throw() { return outStream << '\n'; }
...
void CStdOutStream::NormalizePrint_UString(const UString& s) { NormalizePrint_wstr(s); }
