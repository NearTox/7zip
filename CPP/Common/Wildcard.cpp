...
bool IsPath1PrefixedByPath2(const wchar_t* s1, const wchar_t* s2) {
  if (g_CaseSensitive) return IsString1PrefixedByString2(s1, s2);
  return IsString1PrefixedByString2_NoCase(s1, s2);
}

int CompareFileNames(const wchar_t* s1, const wchar_t* s2) STRING_UNICODE_THROW {
  if (g_CaseSensitive) return MyStringCompare(s1, s2);
...
int CompareFileNames(const char* s1, const char* s2) {
  ...
  if (g_CaseSensitive) return MyStringCompare(u1, u2);
...
static bool EnhancedMaskTest(const wchar_t* mask, const wchar_t* name) {
  for (;;) {
    ...
    if (m == 0) return (c == 0);
    if (m == '*') {
      if (EnhancedMaskTest(mask + 1, name)) return true;
      if (c == 0) return false;
    } else {
      if (m == '?') {
        if (c == 0) return false;
      } else if (m != c)
        if (g_CaseSensitive || MyCharUpper(m) != MyCharUpper(c)) return false;
      mask++;
    }
    name++;
  }
...
bool CItem::AreAllAllowed() const {
  return ForFile && ForDir && WildcardMatching && PathParts.Size() == 1 &&
         PathParts.Front() == L"*";
}
  ...
  */
  ...
  for (int d = start; d <= finish; d++) {
    ...
    for (i = 0; i < PathParts.Size(); i++) {
      if (WildcardMatching) {
        if (!DoesWildcardMatchName(PathParts[i], pathParts[i + d])) break;
      } else {
        if (CompareFileNames(PathParts[i], pathParts[i + d]) != 0) break;
      }
    }
    if (i == PathParts.Size()) return true;
  }
  return false;
}

bool CCensorNode::AreAllAllowed() const {
  if (!Name.IsEmpty() || !SubNodes.IsEmpty() || !ExcludeItems.IsEmpty() || IncludeItems.Size() != 1)
    return false;
...
void CCensorNode::AddItem(bool include, CItem& item, int ignoreWildcardIndex) {
  if (item.PathParts.Size() <= 1) {
    if (item.PathParts.Size() != 0 && item.WildcardMatching) {
      if (!DoesNameContainWildcard(item.PathParts.Front())) item.WildcardMatching = false;
    }
  ...
  if (item.WildcardMatching && ignoreWildcardIndex != 0 && DoesNameContainWildcard(front)) {
    AddItemSimple(include, item);
    return;
  }
  int index = FindSubNode(front);
  if (index < 0) index = SubNodes.Add(CCensorNode(front, this));
  item.PathParts.Delete(0);
  SubNodes[index].AddItem(include, item, ignoreWildcardIndex - 1);
}

void CCensorNode::AddItem(
    bool include, const UString& path, bool recursive, bool forFile, bool forDir,
...
bool CCensorNode::NeedCheckSubDirs() const {
  FOR_VECTOR(i, IncludeItems) {
    ...
    if (item.Recursive || item.PathParts.Size() > 1) return true;
  }
  return false;
}

bool CCensorNode::AreThereIncludeItems() const {
  if (IncludeItems.Size() > 0) return true;
  FOR_VECTOR(i, SubNodes)
  if (SubNodes[i].AreThereIncludeItems()) return true;
  return false;
}

bool CCensorNode::CheckPathCurrent(
    bool include, const UStringVector& pathParts, bool isFile) const {
  const CObjectVector<CItem>& items = include ? IncludeItems : ExcludeItems;
  FOR_VECTOR(i, items)
  if (items[i].CheckPath(pathParts, isFile)) return true;
  return false;
}
...
bool CCensorNode::CheckPathVect(const UStringVector& pathParts, bool isFile, bool& include) const {
  ...
  if (pathParts.Size() <= 1) return finded;
  int index = FindSubNode(pathParts.Front());
  if (index >= 0) {
    UStringVector pathParts2 = pathParts;
    pathParts2.Delete(0);
    if (SubNodes[index].CheckPathVect(pathParts2, isFile, include)) return true;
  }
  return finded;
}

/*
bool CCensorNode::CheckPath2(bool isAltStream, const UString &path, bool isFile, bool &include)
const
{
...
bool CCensorNode::CheckPathToRoot(bool include, UStringVector& pathParts, bool isFile) const {
  if (CheckPathCurrent(include, pathParts, isFile)) return true;
...
void CCensorNode::ExtendExclude(const CCensorNode& fromNodes) {
  ...
  FOR_VECTOR(i, fromNodes.SubNodes) {
    ...
    if (subNodeIndex < 0) subNodeIndex = SubNodes.Add(CCensorNode(node.Name, this));
...
unsigned GetNumPrefixParts_if_DrivePath(UStringVector& pathParts) {
  ...
  if (pathParts[0].IsEmpty()) {
    if (pathParts.Size() < 4 || !pathParts[1].IsEmpty() || pathParts[2] != L"?") return 0;
    testIndex = 3;
  }
  if (NWildcard::IsDriveColonName(pathParts[testIndex])) return testIndex + 1;
  return 0;
}
...
static unsigned GetNumPrefixParts(const UStringVector& pathParts) {
  ...
  if (!pathParts[0].IsEmpty()) return 0;

  if (pathParts.Size() == 1) return 1;
  if (!pathParts[1].IsEmpty()) return 1;
  if (pathParts.Size() == 2) return 2;
  if (pathParts[2] == L".") return 3;

  unsigned networkParts = 2;
  if (pathParts[2] == L"?") {
    if (pathParts.Size() == 3) return 3;
    if (IsDriveColonName(pathParts[3])) return 4;
    if (!pathParts[3].IsEqualTo_Ascii_NoCase("UNC")) return 3;
    networkParts = 4;
  }
  ...
  networkParts +=
      // 2; // server/share
      1;  // server
  if (pathParts.Size() <= networkParts) return pathParts.Size();
  return networkParts;
...
void CCensor::AddItem(
    ECensorPathMode pathMode, bool include, const UString& path, bool recursive,
    bool wildcardMatching) {
  if (path.IsEmpty()) throw "Empty file path";
  ...
  if (pathParts.Size() >= 3 && pathParts[0].IsEmpty() && pathParts[1].IsEmpty() &&
      pathParts[2] == L"?")
  ...
  if (index < 0) index = Pairs.Add(CPair(prefix));
