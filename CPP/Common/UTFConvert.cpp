...
/*
  _UTF8_START(n) - is a base value for start byte (head), if there are (n) additional bytes after
  start byte
...
#define _UTF8_HEAD_PARSE                                                      \
  _UTF8_HEAD_PARSE2(1)                                                        \
  else _UTF8_HEAD_PARSE2(2) else _UTF8_HEAD_PARSE2(3) else _UTF8_HEAD_PARSE2( \
      4) else _UTF8_HEAD_PARSE2(5)
...
bool CheckUTF8(const char* src, bool allowReduced) throw() {
  for (;;) {
    ...
    do {
      Byte c2 = *src++;
      if (c2 < 0x80 || c2 >= 0xC0) return allowReduced && c2 == 0;
      val <<= 6;
      val |= (c2 - 0x80);
    } while (--numBytes);

    if (val >= 0x110000) return false;
  }
}

#define _ERROR_UTF8                            \
  {                                            \
    if (dest) dest[destPos] = (wchar_t)0xFFFD; \
    destPos++;                                 \
    ok = false;                                \
    continue;                                  \
  }
...
static bool Utf8_To_Utf16(
    wchar_t* dest, size_t* destLen, const char* src, const char* srcLim) throw() {
  size_t destPos = 0;
  bool ok = true;
  ...
  for (;;) {
    ...
    if (c < 0x80) {
      if (dest) dest[destPos] = (wchar_t)c;
      destPos++;
      continue;
    }
    if (c < 0xC0) _ERROR_UTF8
    ...
    if (numBytes != 0) _ERROR_UTF8

    if (val < 0x10000) {
      if (dest) dest[destPos] = (wchar_t)val;
      destPos++;
    } else {
      val -= 0x10000;
      if (val >= 0x100000) _ERROR_UTF8
      if (dest) {
