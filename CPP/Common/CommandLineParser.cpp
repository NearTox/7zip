...
void SplitCommandLine(const UString& s, UStringVector& parts) {
  ...
  for (;;) {
    UString s1, s2;
    if (SplitCommandLine(sTemp, s1, s2)) parts.Add(s1);
    if (s2.IsEmpty()) break;
    sTemp = s2;
  }
}
...
bool CParser::ParseString(const UString& s, const CSwitchForm* switchForms, unsigned numSwitches) {
  if (s.IsEmpty() || !IsItSwitchChar(s[0])) return false;
  ...
  for (unsigned i = 0; i < numSwitches; i++) {
    ...
    if ((int)switchLen <= maxLen || pos + switchLen > s.Len()) continue;
...
bool CParser::ParseStrings(
    const CSwitchForm* switchForms, unsigned numSwitches, const UStringVector& commandStrings) {
  StopSwitchIndex = -1;
  ErrorMessage.Empty();
  ErrorLine.Empty();
  ...
  FOR_VECTOR(i, commandStrings) {
    ...
    if (StopSwitchIndex < 0) {
      ...
      if (!s.IsEmpty() && IsItSwitchChar(s[0])) {
        if (ParseString(s, switchForms, numSwitches)) continue;
        ErrorLine = s;
        return false;
      }
    }
