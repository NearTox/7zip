...
void ConvertUInt32ToString(UInt32 val, char* s) throw() { CONVERT_INT_TO_STR(char, 16); }
...
void ConvertUInt32ToString(UInt32 val, wchar_t* s) throw() { CONVERT_INT_TO_STR(wchar_t, 16); }
...
char* RawLeGuidToString(const Byte* g, char* s) throw() {
  ConvertUInt32ToHex8Digits(GetUi32(g), s);
  s += 8;
  *s++ = '-';
  ConvertUInt16ToHex4Digits(GetUi16(g + 4), s);
  s += 4;
  *s++ = '-';
  ConvertUInt16ToHex4Digits(GetUi16(g + 6), s);
  s += 4;
  *s++ = '-';
