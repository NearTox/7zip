...
class CRecordVector {
  ...
  void ReserveOnePosition() {
    if (_size == _capacity) {
      ...
      // p = new T[newCapacity];
      if (_size != 0) memcpy(p, _items, (size_t)_size * sizeof(T));
      delete[] _items;
      _items = p;
  ...
  */
  ...
  CRecordVector& operator=(const CRecordVector& v) {
    ...
    _size = size;
    if (size != 0) memcpy(_items, v._items, (size_t)size * sizeof(T));
    return *this;
  }
  ...
  CRecordVector& operator+=(const CRecordVector& v) {
    unsigned size = v.Size();
    Reserve(_size + size);
    if (size != 0) memcpy(_items + _size, v._items, (size_t)size * sizeof(T));
    _size += size;
    return *this;
  }
  ...
  void AddInReserved(const T item) { _items[_size++] = item; }
  ...
  */
  ...
  static void SortRefDown(
      T* p, unsigned k, unsigned size, int (*compare)(const T*, const T*, void*), void* param) {
    T temp = p[k];
    for (;;) {
      unsigned s = (k << 1);
      if (s > size) break;
      if (s < size && compare(p + s + 1, p + s, param) > 0) s++;
      if (compare(&temp, p + s, param) >= 0) break;
      p[k] = p[s];
      k = s;
    }
    p[k] = temp;
  ...
  static void SortRefDown2(T* p, unsigned k, unsigned size) {
    ...
    for (;;) {
      unsigned s = (k << 1);
      if (s > size) break;
      if (s < size && p[(size_t)s + 1].Compare(p[s]) > 0) s++;
      if (temp.Compare(p[s]) >= 0) break;
      p[k] = p[s];
      k = s;
    }
    p[k] = temp;
...
class CObjectVector {
 ...
 public:
  ...
  CObjectVector(const CObjectVector& v) {
    ...
    for (unsigned i = 0; i < size; i++) _v.AddInReserved(new T(v[i]));
  }
  ...
  ~CObjectVector() {
    for (unsigned i = _v.Size(); i != 0;) delete (T*)_v[--i];
  }
  ...
  void DeleteFrom(unsigned index) {
    ...
    for (unsigned i = index; i < size; i++) delete (T*)_v[i];
    _v.DeleteFrom(index);
  }

  void DeleteFrontal(unsigned num) {
    for (unsigned i = 0; i < num; i++) delete (T*)_v[i];
