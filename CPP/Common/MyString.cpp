...
// const char * MyStringGetPrevCharPointer(const char *base, const char *p) throw() { return
// CharPrevA(base, p); } char * MyStringUpper(char *s) { return CharUpperA(s); } char *
// MyStringLower(char *s) { return CharLowerA(s); }
...
wchar_t MyCharUpper_WIN(wchar_t c) throw() {
  ...
  if (numChars == 0 || numChars > kBufSize) return c;
  s[numChars] = 0;
  ::CharUpperA(s);
...
bool AString::IsPrefixedBy_Ascii_NoCase(const char* s) const throw() {
  ...
  for (;;) {
    ...
    if (c2 == 0) return true;
    char c1 = *s1++;
    if (MyCharLower_Ascii(c1) != MyCharLower_Ascii(c2)) return false;
  }
}
...
bool UString::IsPrefixedBy_Ascii_NoCase(const char* s) const throw() {
  ...
  for (;;) {
    ...
    if (c2 == 0) return true;
    wchar_t c1 = *s1++;
    if (MyCharLower_Ascii(c1) != (unsigned char)MyCharLower_Ascii(c2)) return false;
  }
}
...
bool StringsAreEqualNoCase_Ascii(const char* s1, const char* s2) throw() {
  for (;;) {
    char c1 = *s1++;
    char c2 = *s2++;
    if (c1 != c2 && MyCharLower_Ascii(c1) != MyCharLower_Ascii(c2)) return false;
    if (c1 == 0) return true;
  }
}
...
bool StringsAreEqualNoCase_Ascii(const wchar_t* s1, const char* s2) throw() {
  for (;;) {
    ...
    if (c1 != (unsigned char)c2 &&
        (c1 > 0x7F || MyCharLower_Ascii(c1) != (unsigned char)MyCharLower_Ascii(c2)))
      return false;
    if (c1 == 0) return true;
  }
}
...
AString operator+(const AString& s1, const AString& s2) {
  return AString(s1, s1.Len(), s2, s2.Len());
}
AString operator+(const AString& s1, const char* s2) {
  return AString(s1, s1.Len(), s2, MyStringLen(s2));
}
AString operator+(const char* s1, const AString& s2) {
  return AString(s1, MyStringLen(s1), s2, s2.Len());
}
...
void AString::SetFromWStr_if_Ascii(const wchar_t* s) {
  ...
  char* dest = _chars;
  unsigned i;
  for (i = 0; i < len; i++) dest[i] = (char)s[i];
  dest[i] = 0;
}
...
void AString::Add_Space_if_NotEmpty() {
  if (!IsEmpty()) Add_Space();
}
...
int AString::ReverseFind(char c) const throw() {
  ...
  for (;;) {
    if (*p == c) return (int)(p - _chars);
    if (p == _chars) return -1;
...
int AString::ReverseFind_PathSepar() const throw() {
  ...
  for (;;) {
    char c = *p;
    if (IS_PATH_SEPAR(c)) return (int)(p - _chars);
    if (p == _chars) return -1;
    p--;
  }
}
...
UString operator+(const UString& s1, const UString& s2) {
  return UString(s1, s1.Len(), s2, s2.Len());
}
UString operator+(const UString& s1, const wchar_t* s2) {
  return UString(s1, s1.Len(), s2, MyStringLen(s2));
}
UString operator+(const wchar_t* s1, const UString& s2) {
  return UString(s1, MyStringLen(s1), s2, s2.Len());
}
...
UString::UString(const char* s) {
  ...
  SetStartLen(len);
  wchar_t* chars = _chars;
  for (unsigned i = 0; i < len; i++) chars[i] = (unsigned char)s[i];
  chars[len] = 0;
}
...
UString2& UString2::operator=(const wchar_t* s) {
  ...
  if (len > _len) {
    wchar_t* newBuf = MY_STRING_NEW_wchar_t(len + 1);
    if (_chars) MY_STRING_DELETE(_chars);
    _chars = newBuf;
  }
  _len = len;
  MyStringCopy(_chars, s);
