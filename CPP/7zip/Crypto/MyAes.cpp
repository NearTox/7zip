...
STDMETHODIMP_(UInt32) CAesCbcCoder::Filter(Byte* data, UInt32 size) {
  ...
  if (size == 0) return 0;
  if (size < AES_BLOCK_SIZE) return AES_BLOCK_SIZE;
  size >>= 4;
...
STDMETHODIMP CAesCbcCoder::SetKey(const Byte* data, UInt32 size) {
  if ((size & 0x7) != 0 || size < 16 || size > 32) return E_INVALIDARG;
  if (_keySize != 0 && size != _keySize) return E_INVALIDARG;
...
STDMETHODIMP CAesCbcCoder::SetInitVector(const Byte* data, UInt32 size) {
  if (size != AES_BLOCK_SIZE) return E_INVALIDARG;
...
bool CAesCbcCoder::SetFunctions(UInt32 algo) {
  ...
  if (algo == 1) { _codeFunc = _encodeMode ? AesCbc_Encode : AesCbc_Decode; }
  if (algo == 2) {
...
STDMETHODIMP CAesCbcCoder::SetCoderProperties(
    const PROPID* propIDs, const PROPVARIANT* coderProps, UInt32 numProps) {
  for (UInt32 i = 0; i < numProps; i++) {
    const PROPVARIANT& prop = coderProps[i];
    if (propIDs[i] == NCoderPropID::kDefaultProp) {
      if (prop.vt != VT_UI4) return E_INVALIDARG;
      if (!SetFunctions(prop.ulVal)) return E_NOTIMPL;
    }
  }
  return S_OK;
}
