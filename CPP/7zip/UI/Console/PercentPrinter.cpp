...
void CPercentPrinter::Print() {
  ...
  if (!_printedString.IsEmpty()) {
    if (_tickStep != 0 && (UInt32)(tick - _prevTick) < _tickStep) return;

    CPercentPrinterState& st = *this;
    if (_printedState.Command == st.Command && _printedState.FileName == st.FileName &&
        _printedState.Files == st.Files) {
      if (_printedState.Total == st.Total && _printedState.Completed == st.Completed) return;
  ...
  if (onlyPercentsChanged && _s == _printedPercents) return;
  ...
  if (!FileName.IsEmpty() && _s.Len() < MaxLen) {
    ...
    if (_s.Len() + _temp.Len() > MaxLen) {
      ...
      for (; len != 0;) {
        ...
        _so->Normalize_UString(_tempU);
        StdOut_Convert_UString_to_AString(_tempU, _temp);
        if (_s.Len() + _temp.Len() <= MaxLen) break;
      }
      if (len == 0) _temp.Empty();
    }
    _s += _temp;
  }
