...
HRESULT COpenCallbackConsole::Open_CheckBreak() { return CheckBreak2(); }
...
HRESULT COpenCallbackConsole::Open_SetTotal(const UInt64* files, const UInt64* bytes) {
  if (!MultiArcMode && NeedPercents()) {
    ...
    if (bytes) {
      ...
      _totalBytes = *bytes;
      if (!files) _percent.Total = *bytes;
    } else {
      // _totalBytesDefined = false;
      if (!files) _percent.Total = _totalBytes;
    }
  }
...
HRESULT COpenCallbackConsole::Open_SetCompleted(const UInt64* files, const UInt64* bytes) {
  if (!MultiArcMode && NeedPercents()) {
    if (files) {
      ...
      if (_totalFilesDefined) _percent.Completed = *files;
    }

    if (bytes) {
      if (!_totalFilesDefined) _percent.Completed = *bytes;
    }
    _percent.Print();
  }
