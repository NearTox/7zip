...
static void PrintStringToString(
    char* dest, EAdjustment adj, unsigned width, const char* textString) {
  unsigned numSpaces = 0;
...
void CFieldPrinter::Init(const CFieldInfoInit* standardFieldTable, unsigned numItems) {
  ...
  for (unsigned i = 0; i < numItems; i++) {
    ...
    unsigned k;
    for (k = 0; k < fii.PrefixSpacesWidth; k++) LinesString.Add_Space();
    for (k = 0; k < fii.Width; k++) LinesString += '-';
  }
}
...
void CFieldPrinter::PrintTitleLines() { g_StdOut << LinesString; }
...
static void PrintNumber(EAdjustment adj, unsigned width, const CListUInt64Def& value) {
  char s[32];
  s[0] = 0;
  if (value.Def) ConvertUInt64ToString(value.Val, s);
...
void CFieldPrinter::PrintSum(const CListStat& st, UInt64 numDirs, const char* str) {
  FOR_VECTOR(i, _fields) {
    ...
    if (f.PropID == kpidSize)
      ...
      char s[64];
      s[0] = 0;
      if (st.MTime.Def) PrintTime(s, &st.MTime.Val);
...
static HRESULT GetUInt64Value(
    IInArchive* archive, UInt32 index, PROPID propID, CListUInt64Def& value) {
  value.Val = 0;
  value.Def = false;
  CPropVariant prop;
...
HRESULT ListArchives(
    ...
    bool stdInMode, UStringVector& arcPaths, UStringVector& arcPathsFull, bool processAltStreams,
    bool showAltStreams, const NWildcard::CCensorNode& wildcardCensor, bool enableHeaders,
    bool techMode,
#ifndef _NO_CRYPTO
  ...
  CFieldPrinter fp;
  if (!techMode) fp.Init(kStandardFieldTable, ARRAY_SIZE(kStandardFieldTable));

  CListStat2 stat2total;

  CBoolArr skipArcs(arcPaths.Size());
  unsigned arcIndex;
  for (arcIndex = 0; arcIndex < arcPaths.Size(); arcIndex++) skipArcs[arcIndex] = false;
  UInt64 numVolumes = 0;
  UInt64 numArcs = 0;
  ...
  for (arcIndex = 0; arcIndex < arcPaths.Size(); arcIndex++) {
    ...
    */
    ...
    if (result != S_OK) {
      ...
      if (result != S_FALSE) lastError = result;
      g_StdOut.Flush();
      if (g_ErrStream) {
        *g_ErrStream << endl << kError;
        g_ErrStream->NormalizePrint_UString(arcPath);
...

    {
      FOR_VECTOR(r, arcLink.Arcs) {
        ...
        if (!arc.WarningMessage.IsEmpty()) numWarnings++;
        ...
        if (!arc.ErrorMessage.IsEmpty()) numErrors++;
      }
    }
    ...
    if (!stdInMode) {
      ...
      FOR_VECTOR(v, arcLink.VolumePaths) {
        ...
        if (index >= 0 && (unsigned)index > arcIndex) skipArcs[(unsigned)index] = true;
      }
    }
    ...
    if (techMode) {
      ...
      if (arc.GetRawProps) { RINOK(fp.AddRawProps(arc.GetRawProps)); }
    }
    ...
    for (UInt32 i = 0; i < numItems; i++) {
      ...
      if (stdInMode && res == E_INVALIDARG) break;
      RINOK(res);
      ...
      if (!allFilesAreAllowed) {
        if (isAltStream) {
          ...
          if (!CensorNode_CheckPath(wildcardCensor, item)) continue;
        } else {
          SplitPathToParts(fp.FilePath, pathParts);
          ;
          bool include;
          if (!wildcardCensor.CheckPathVect(pathParts, !fp.IsDir, include)) continue;
          if (!include) continue;
        }
      }
    ...
    if (!stdInMode && !stat2.MainFiles.PackSize.Def && !stat2.AltStreams.PackSize.Def) {
      if (arcLink.VolumePaths.Size() != 0) arcPackSize += arcLink.VolumesSize;
    ...
    if (enableHeaders) {
      if (arcLink.NonOpen_ErrorInfo.ErrorFormatIndex >= 0) {
        ...
        PrintArcTypeError(
            g_StdOut, codecs->Formats[arcLink.NonOpen_ErrorInfo.ErrorFormatIndex].Name, false);
      }
    }
  ...
  if (numErrors == 1 && lastError != 0) return lastError;
