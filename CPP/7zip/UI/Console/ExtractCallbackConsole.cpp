...
void CExtractScanConsole::StartScanning() {
  if (NeedPercents()) _percent.Command = "Scan";
}

HRESULT CExtractScanConsole::ScanProgress(
    const CDirItemsStat& st, const FString& path, bool /* isDir */) {
  if (NeedPercents()) {
...
void Print_DirItemsStat2(AString& s, const CDirItemsStat2& st) {
  ...
  if (st.Anti_NumDirs != 0) {
    ...
    Print_UInt64_and_String(
        s, st.Anti_NumDirs, st.Anti_NumDirs == 1 ? "anti-folder" : "anti-folders");
  }
  if (st.Anti_NumFiles != 0) {
    ...
    needLF = false;
    Print_UInt64_and_String(
        s, st.Anti_NumFiles, st.Anti_NumFiles == 1 ? "anti-file" : "anti-files");
  }
...
STDMETHODIMP CExtractCallbackConsole::SetCompleted(const UInt64* completeValue) {
  ...
  if (NeedPercents()) {
    if (completeValue) _percent.Completed = *completeValue;
    _percent.Print();
  }
  return CheckBreak2();
}
...
static void PrintFileInfo(
    CStdOutStream* _so, const wchar_t* path, const FILETIME* ft, const UInt64* size) {
  *_so << kTab << "Path:     ";
...
STDMETHODIMP CExtractCallbackConsole::AskOverwrite(
  ...
  *answer = NOverwriteAnswer::kYesToAll;
  ...
  if (_so) {
    *_so << endl;
    if (NeedFlush) _so->Flush();
  }

  return CheckBreak2();
}

STDMETHODIMP CExtractCallbackConsole::PrepareOperation(
  ...
  if (NeedPercents()) {
    if (PercentsNameLevel >= 1) {
      ...
      if (PercentsNameLevel > 1 || !show2) {
        _percent.Command = s;
        if (name) _percent.FileName = name;
      }
    }
    _percent.Print();
  }
...
void SetExtractErrorMessage(Int32 opRes, Int32 encrypted, AString& dest) {
  ...
  switch (opRes) {
    ...
    case NArchive::NExtract::NOperationResult::kDataError:
      ...
      break;
    case NArchive::NExtract::NOperationResult::kUnavailable: s = kUnavailableData; break;
    case NArchive::NExtract::NOperationResult::kUnexpectedEnd: s = kUnexpectedEnd; break;
    case NArchive::NExtract::NOperationResult::kDataAfterEnd: s = kDataAfterEnd; break;
    case NArchive::NExtract::NOperationResult::kIsNotArc: s = kIsNotArc; break;
    case NArchive::NExtract::NOperationResult::kHeadersError: s = kHeadersError; break;
    case NArchive::NExtract::NOperationResult::kWrongPassword: s = kWrongPassword; break;
  }
...
STDMETHODIMP CExtractCallbackConsole::ReportExtractResult(
    Int32 opRes, Int32 encrypted, const wchar_t* name) {
  if (opRes != NArchive::NExtract::NOperationResult::kOK) {
    _currentName = name;
...
HRESULT CExtractCallbackConsole::BeforeOpen(const wchar_t* name, bool testMode) {
  ...
  if (NeedPercents()) _percent.Command = "Open";
  return S_OK;
}
  ...
  *_so << "WARNING:\n";
  ...
  if (arc.FormatIndex == er.ErrorFormatIndex) {
    ...
    Add_Messsage_Pre_ArcType(
        s, "Can not open the file", codecs->GetFormatNamePtr(er.ErrorFormatIndex));
...
HRESULT CExtractCallbackConsole::OpenResult(
  ...
  FOR_VECTOR(level, arcLink.Arcs) {
    ...
    if (errorFlags != 0 || !er.ErrorMessage.IsEmpty()) {
      ...
      if (errorFlags != 0) {
        if (_se) PrintErrorFlags(*_se, "ERRORS:", errorFlags);
        NumOpenArcErrors++;
        ThereIsError_in_Current = true;
      }

      if (!er.ErrorMessage.IsEmpty()) {
        if (_se) *_se << "ERRORS:" << endl << er.ErrorMessage << endl;
        NumOpenArcErrors++;
        ThereIsError_in_Current = true;
      }
    ...
    if (warningFlags != 0 || !er.WarningMessage.IsEmpty()) {
      ...
      if (warningFlags != 0) {
        if (_so) PrintErrorFlags(*_so, "WARNINGS:", warningFlags);
        NumOpenArcWarnings++;
        ThereIsWarning_in_Current = true;
      }

      if (!er.WarningMessage.IsEmpty()) {
        if (_so) *_so << "WARNINGS:" << endl << er.WarningMessage << endl;
        NumOpenArcWarnings++;
        ThereIsWarning_in_Current = true;
      }
...
HRESULT CExtractCallbackConsole::ExtractResult(HRESULT result) {
  ...
  if (result == S_OK) {
    ...
    if (_so && NeedFlush) _so->Flush();
  } else {
    NumArcsWithError++;
    if (result == E_ABORT || result == ERROR_DISK_FULL) return result;
