...
CArchiveUpdateCallback::CArchiveUpdateCallback() :
  ...
  ProcessedItemsStatuses(nullptr) {
#ifdef _USE_SECURITY_CODE
      ...
      if(inStreamSpec->GetProps2(&props) == S_OK) {
        if(props.NumLinks > 1) {
          ...
          if(numItems == _map.Size()) {
            ...
            // we could return nullptr as stream, but it's better to return real stream
            // return S_OK;
          }
        }
      }
...
STDMETHODIMP CArchiveUpdateCallback::ReportOperation(UInt32 indexType, UInt32 index, UInt32 op) {
  ...
  if(indexType == NArchive::NEventIndexType::kOutArcIndex) {
    ...
    return Callback->ReportUpdateOpeartion(op, name.IsEmpty() ? nullptr : name.Ptr(), isDir);
  }
