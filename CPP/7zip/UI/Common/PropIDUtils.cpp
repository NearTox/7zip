...
static const char kPosixTypes[16] = {'0', 'p', 'c', '3', 'd', '5', 'b', '7',
                                     '-', '9', 'l', 'B', 's', 'D', 'E', 'F'};
...
void ConvertPropertyToShortString2(
    char* dest, const PROPVARIANT& prop, PROPID propID, int level) throw() {
  *dest = 0;
...
static inline unsigned GetHex(unsigned v) { return (v < 10) ? ('0' + v) : ('A' + (v - 10)); }
...
static const char* const sidNames[] = {"0",
                                       "Dialup",
                                       "Network",
                                       "Batch",
                                       "Interactive",
...
static int FindPairIndex(const CSecID2Name* pairs, unsigned num, UInt32 id) {
  for (unsigned i = 0; i < num; i++)
    if (pairs[i].n == id) return i;
  return -1;
}

static const CSecID2Name sid_32_Names[] = {{544, "Administrators"},
                                           {545, "Users"},
                                           {546, "Guests"},
                                           {547, "PowerUsers"},
...
static const CSecID2Name sid_21_Names[] = {{500, "Administrator"},
                                           {501, "Guest"},
                                           {502, "KRBTGT"},
                                           {512, "DomainAdmins"},
                                           {513, "DomainUsers"},
...
static void ParseSid(AString& s, const Byte* p, UInt32 lim, UInt32& sidSize) {
  ...
  if (p[2] == 0 && p[3] == 0)
    ...
    s += "0x";
    for (int i = 2; i < 8; i++) AddHexToString(s, p[i]);
  }
...
static void ParseAcl(
    AString& s, const Byte* p, UInt32 size, const char* strName, UInt32 flags, UInt32 offset) {
  UInt32 control = Get16(p + 2);
  if ((flags & control) == 0) return;
...
static bool CheckAcl(const Byte* p, UInt32 size, UInt32 flags, UInt32 offset) throw() {
  UInt32 control = Get16(p + 2);
  if ((flags & control) == 0) return true;
  UInt32 pos = Get32(p + offset);
  if (pos >= size) return false;
  p += pos;
  size -= pos;
...
static const CSecID2Name k_ReparseTags[] = {{0xA0000003, "MOUNT_POINT"},
                                            {0xC0000004, "HSM"},
