...
struct CBenchBuffer {
  ...
  CBenchBuffer() : BufferSize(0), Buffer(nullptr) {}
  ...
  CBenchBuffer() : BufferSize(0), Buffer(nullptr), _bufBase(nullptr) {}
...
static UInt64 GetTimeCount() {
  ...
  return (UInt64)time(nullptr) * 1000000;
#else
  return time(nullptr);
#endif
#else
...

    {
      ...
      if(sp) {
        ...
        if(_encoderFilter) {
          ...
          RINOK(_encoder->Code(inStream, crcStream, 0, 0, nullptr));
        }
      }
    }
  }
...
HRESULT CEncoderInfo::Encode() {
  ...
  for(UInt64 i = 0; i < NumIterations; i++) {
    ...
    if(_encoderFilter) {
      ...
      RINOK(_encoder->Code(inStream, outStream, nullptr, nullptr, progressInfo[0]));
    }
...
static THREAD_FUNC_DECL CrcThreadFunction(void *param) {
  ...
  p->Res = CrcBig(p->Data, p->Size, p->NumIterations,
    p->CheckSumDefined ? &p->CheckSum : nullptr, p->Hasher,
    p->Callback);
  return 0;
}
  ...
  if(_file) {
    {
      ...
      PrintResults(_file, info,
        0, // weight
        rating,
        showFreq, showFreq ? (specifiedFreq != 0 ? specifiedFreq : cpuFreq) : 0, nullptr);
    }
    RINOK(_file->CheckBreak());
  }
      ...
      for(unsigned pow = 10; pow < 32; pow++) {
        ...
        for(unsigned ti = 0; ti < numThreadsTests; ti++) {
          ...
          RINOK(CrcBench(EXTERNAL_CODECS_LOC_VARS complexInCommands,
            ...
            complexity,
            1, // benchWeight,
            (pow == kNumHashDictBits) ? checkSum : nullptr, method, nullptr, nullptr, false, 0));
