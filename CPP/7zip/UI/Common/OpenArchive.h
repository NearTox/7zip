...
struct COpenType {
  ...
  const COpenSpecFlags& GetSpec(bool isForced, bool isMain, bool isUnknown) const {
    return isForced ? SpecForcedType
...
struct CArcErrorInfo {
  ...
  bool IsThereErrorOrWarning() const {
    return ErrorFlags != 0 || WarningFlags != 0 || NeedTailWarning() || UnexpecedEnd ||
           !ErrorMessage.IsEmpty() || !WarningMessage.IsEmpty();
  }
  ...
  UInt32 GetErrorFlags() const {
    UInt32 a = ErrorFlags;
    if (UnexpecedEnd) a |= kpv_ErrorFlags_UnexpectedEnd;
    return a;
  }
};
...
struct CReadArcItem {
  ...
  UStringVector PathParts;  // without altStream name, path from root or from _baseParentFolder, if
...
struct CArchiveLink {
  ...
  CArchiveLink() : VolumesSize(0), IsOpen(false), PasswordWasAsked(false) {}
  ...
  HRESULT Open_Strict(COpenOptions& options, IOpenCallbackUI* callbackUI) {
    ...
    if (result == S_OK && NonOpen_ErrorInfo.ErrorFormatIndex >= 0) result = S_FALSE;
    return result;
  }
