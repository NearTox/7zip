...
STDMETHODIMP COutMultiVolStream::Write(const void *data, UInt32 size, UInt32 *processedSize) {
  ...
  while(size > 0) {
    ...
    if(_offsetPos != altStream.Pos) {
      ...
      RINOK(altStream.Stream->Seek(_offsetPos, STREAM_SEEK_SET, nullptr));
...
static HRESULT Compress(
  ...
  if(options.RenamePairs.Size() != 0) {
    ...
    UpdateProduce(updatePairs, actionSet, updatePairs2, isUpdatingItself ? &upCallback : nullptr);
  }
  ...
  COutFileStream *outStreamSpec = nullptr;
  CStdOutFileStream *stdOutFileStreamSpec = nullptr;
  COutMultiVolStream *volStreamSpec = nullptr;
  ...
  if(options.SfxMode) {
    ...
    COutFileStream *outStreamSpec2 = nullptr;
    if(options.VolumesSizes.Size() == 0)
    ...
    RINOK(NCompress::CopyStream(sfxStream, sfxOutStream, nullptr));
  ...
  if(options.SfxMode || !arc || arc->ArcStreamOffset == 0)
    ...
    RINOK(arc->InStream->Seek(0, STREAM_SEEK_SET, nullptr));
    RINOK(NCompress::CopyStream_ExactSize(arc->InStream, outStream, arc->ArcStreamOffset, nullptr));
    if(options.StdOutMode)
      tailStream = outStream;
    else {
...
HRESULT UpdateArchive(
  ...
  CDirItem *parentDirItem_Ptr = nullptr;

  /*
  FStringVector requestedPaths;
  FStringVector *requestedPaths_Ptr = nullptr;
  if (options.DeleteAfterCompressing)
  ...
  */
  ...
  FStringVector *processedFilePaths_Ptr = nullptr;
  if (options.DeleteAfterCompressing)
  ...
  */
  ...
  for(ci = 0; ci < options.Commands.Size(); ci++) {
    const CArc *arc = thereIsInArchive ? arcLink.GetArc() : nullptr;
    ...
    RINOK(Compress(options,
      ...
      command.ArchivePath,
      arcItems,
      options.DeleteAfterCompressing ? (Byte *)processedItems : nullptr,
