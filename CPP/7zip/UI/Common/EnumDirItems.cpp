...
void CDirItems::AddDirFileInfo(
    int phyParent, int logParent, int secureIndex, const NFind::CFileInfo& fi) {
  CDirItem di;
  di.Size = fi.Size;
  di.CTime = fi.CTime;
  di.ATime = fi.ATime;
...
HRESULT CDirItems::AddError(const FString& path, DWORD errorCode) {
  Stat.NumErrors++;
  if (Callback) return Callback->ScanError(path, errorCode);
  return S_OK;
}

HRESULT CDirItems::AddError(const FString& path) { return AddError(path, ::GetLastError()); }

static const unsigned kScanProgressStepMask = (1 << 12) - 1;

HRESULT CDirItems::ScanProgress(const FString& dirPath) {
  if (Callback) return Callback->ScanProgress(Stat, dirPath, true);
  return S_OK;
}

UString CDirItems::GetPrefixesPath(
    const CIntVector& parents, int index, const UString& name) const {
  UString path;
  unsigned len = name.Len();

  int i;
  for (i = index; i >= 0; i = parents[i]) len += Prefixes[i].Len();
...
HRESULT CDirItems::AddSecurityItem(const FString& path, int& secureIndex) {
  ...
  if (_saclEnabled) securInfo |= SACL_SECURITY_INFORMATION;

  DWORD errorCode = 0;
  DWORD secureSize;

  BOOL res = ::GetFileSecurityW(
      fs2us(path), securInfo, (PSECURITY_DESCRIPTOR)(Byte*)TempSecureBuf,
      (DWORD)TempSecureBuf.Size(), &secureSize);

  if (res) {
    if (secureSize == 0) return S_OK;
    if (secureSize > TempSecureBuf.Size()) errorCode = ERROR_INVALID_FUNCTION;
  } else {
  ...
  if (errorCode == 0) errorCode = ERROR_INVALID_FUNCTION;
...
HRESULT CDirItems::EnumerateDir(int phyParent, int logParent, const FString& phyPrefix) {
  ...
  for (unsigned ttt = 0;; ttt++) {
    NFind::CFileInfo fi;
    bool found;
    if (!enumerator.Next(fi, found)) { return AddError(phyPrefix); }
    if (!found) return S_OK;

    int secureIndex = -1;
#ifdef _USE_SECURITY_CODE
    if (ReadSecure) { RINOK(AddSecurityItem(phyPrefix + fi.Name, secureIndex)); }
#endif
...
HRESULT CDirItems::EnumerateItems2(
    const FString& phyPrefix, const UString& logPrefix, const FStringVector& filePaths,
  ...
  FOR_VECTOR(i, filePaths) {
    ...
    if (requestedPaths) requestedPaths->Add(phyPath);
    ...
    int secureIndex = -1;
#ifdef _USE_SECURITY_CODE
    if (ReadSecure) { RINOK(AddSecurityItem(phyPath, secureIndex)); }
#endif
...
static HRESULT EnumerateDirItems(
    const NWildcard::CCensorNode& curNode, int phyParent, int logParent, const FString& phyPrefix,
    const UStringVector& addArchivePrefix, CDirItems& dirItems, bool enterToSubFolders);

static HRESULT EnumerateDirItems_Spec(
    const NWildcard::CCensorNode& curNode, int phyParent, int logParent,
    const FString& curFolderName, const FString& phyPrefix, const UStringVector& addArchivePrefix,
    CDirItems& dirItems, bool enterToSubFolders) {
  ...
  HRESULT res = EnumerateDirItems(
      curNode, parent, parent, phyPrefix + name2, addArchivePrefix, dirItems, enterToSubFolders);
  if (numItems == dirItems.Items.Size()) dirItems.DeleteLastPrefix();
  return res;
}
...
static HRESULT EnumerateAltStreams(
    const NFind::CFileInfo& fi, const NWildcard::CCensorNode& curNode, int phyParent, int logParent,
    const FString& fullPath,
    const UStringVector& addArchivePrefix,  // prefix from curNode
    bool addAllItems, CDirItems& dirItems) {
  ...
  for (;;) {
    ...
    if (!found) return S_OK;
    if (si.IsMainStream()) continue;
    UStringVector addArchivePrefixNew = addArchivePrefix;
    UString reducedName = si.GetReducedName();
    addArchivePrefixNew.Back() += reducedName;
    if (curNode.CheckPathToRoot(false, addArchivePrefixNew, true)) continue;
    if (!addAllItems)
      if (!curNode.CheckPathToRoot(true, addArchivePrefixNew, true)) continue;
...
HRESULT CDirItems::SetLinkInfo(
    CDirItem& dirItem, const NFind::CFileInfo& fi, const FString& phyPrefix) {
  if (!SymLinks || !fi.HasReparsePoint()) return S_OK;
  const FString path = phyPrefix + fi.Name;
  CByteBuffer& buf = dirItem.ReparseData;
  DWORD res = 0;
  if (NIO::GetReparseData(path, buf)) {
    CReparseAttr attr;
    if (attr.Parse(buf, buf.Size(), res)) return S_OK;
    // we ignore unknown reparse points
    if (res != ERROR_INVALID_REPARSE_DATA) res = 0;
  } else {
    res = ::GetLastError();
    if (res == 0) res = ERROR_INVALID_FUNCTION;
  }
...
static HRESULT EnumerateForItem(
    NFind::CFileInfo& fi, const NWildcard::CCensorNode& curNode, int phyParent, int logParent,
  ...
  addArchivePrefixNew.Add(name);
  {
    ...
    if (curNode.CheckPathToRoot(false, addArchivePrefixNewTemp, !fi.IsDir())) return S_OK;
  }
  int dirItemIndex = -1;
  ...
  if (curNode.CheckPathToRoot(true, addArchivePrefixNew, !fi.IsDir())) {
    int secureIndex = -1;
#ifdef _USE_SECURITY_CODE
    if (dirItems.ReadSecure) { RINOK(dirItems.AddSecurityItem(phyPrefix + fi.Name, secureIndex)); }
#endif

    dirItemIndex = dirItems.Items.Size();
    dirItems.AddDirFileInfo(phyParent, logParent, secureIndex, fi);
    if (fi.IsDir()) enterToSubFolders2 = true;
  ...
  if (dirItems.ScanAltStreams) {
    RINOK(EnumerateAltStreams(
        fi, curNode, phyParent, logParent, phyPrefix + fi.Name, addArchivePrefixNew,
        addAllSubStreams, dirItems));
  }

  if (dirItemIndex >= 0) {
    CDirItem& dirItem = dirItems.Items[dirItemIndex];
    RINOK(dirItems.SetLinkInfo(dirItem, fi, phyPrefix));
    if (dirItem.ReparseData.Size() != 0) return S_OK;
  }
#endif

  if (!fi.IsDir()) return S_OK;

  const NWildcard::CCensorNode* nextNode = 0;
  if (addArchivePrefix.IsEmpty()) {
    int index = curNode.FindSubNode(name);
    if (index >= 0) nextNode = &curNode.SubNodes[index];
  }
  if (!enterToSubFolders2 && nextNode == 0) return S_OK;
  ...
  return EnumerateDirItems_Spec(
      *nextNode, phyParent, logParent, fi.Name, phyPrefix, addArchivePrefixNew, dirItems,
      enterToSubFolders2);
}
...
static bool CanUseFsDirect(const NWildcard::CCensorNode& curNode) {
  FOR_VECTOR(i, curNode.IncludeItems) {
    ...
    if (item.Recursive || item.PathParts.Size() != 1) return false;
    ...
    */
    ...
    if (!item.WildcardParsing)
      continue;
    */
    if (DoesNameContainWildcard(name)) return false;
  }
  return true;
}
    ...
    if (CanUseFsDirect(curNode)) {
      ...
      for (i = 0; i < curNode.IncludeItems.Size(); i++) {
        ...
        }
        {
          ...
          if (curNode.CheckPathToRoot(false, pathParts, !isDir)) continue;
        }
        ...
        if (needAltStreams && dirItems.ScanAltStreams) {
          ...
          pathParts.Add(fs2us(fi.Name));
          RINOK(EnumerateAltStreams(
              fi, curNode, phyParent, logParent, fullPath, pathParts, true, /* addAllSubStreams */
              dirItems));
        }
#endif
        ...
        int index = curNode.FindSubNode(name);
        if (index >= 0) {
          for (int t = needEnterVector.Size(); t <= index; t++) needEnterVector.Add(true);
      ...
      for (i = 0; i < curNode.SubNodes.Size(); i++) {
        ...
        if (phyPrefix.IsEmpty()) {
          {
            if (nextNode.Name.IsEmpty()) fullPath = CHAR_PATH_SEPARATOR;
#ifdef _WIN32
        ...
        RINOK(EnumerateDirItems_Spec(
            nextNode, phyParent, logParent, fi.Name, phyPrefix, UStringVector(), dirItems, false));
      }
...
#  ifndef UNDER_CE
  ...
  if (phyPrefix.IsEmpty() && curNode.IncludeItems.Size() > 0) {
    ...
    for (i = 0; i < curNode.IncludeItems.Size(); i++) {
      const NWildcard::CItem& item = curNode.IncludeItems[i];
      if (item.PathParts.Size() < 1) break;
      const UString& name = item.PathParts.Front();
      if (name.Len() != 2 || name[1] != ':') break;
      if (item.PathParts.Size() == 1)
        if (item.ForFile || !item.ForDir) break;
      if (NWildcard::IsDriveColonName(name)) continue;
    ...
    if (i == curNode.IncludeItems.Size()) {
      ...
      for (i = 0; i < driveStrings.Size(); i++) {
        ...
        if (driveName.Len() < 3 || driveName.Back() != '\\') return E_FAIL;
        driveName.DeleteBack();
        NFind::CFileInfo fi;
        fi.SetAsDir();
        fi.Name = driveName;

        RINOK(EnumerateForItem(
            fi, curNode, phyParent, logParent, phyPrefix, addArchivePrefix, dirItems,
            enterToSubFolders));
      }
      return S_OK;
    }
...
HRESULT EnumerateItems(
    const NWildcard::CCensor& censor, const NWildcard::ECensorPathMode pathMode,
  ...
  FOR_VECTOR(i, censor.Pairs) {
    ...
    if (pathMode == NWildcard::k_AbsPath)
      ...
      if (!addPathPrefix.IsEmpty()) logParent = dirItems.AddPrefix(-1, -1, addPathPrefix);
    }

    RINOK(EnumerateDirItems(
        pair.Head, phyParent, logParent, us2fs(pair.Prefix), UStringVector(), dirItems,
...
void CDirItems::FillFixedReparse() {
  ...
  FOR_VECTOR(i, Items) {
    ...
    if (item.ReparseData.Size() == 0) continue;

    CReparseAttr attr;
    DWORD errorCode = 0;
    if (!attr.Parse(item.ReparseData, item.ReparseData.Size(), errorCode)) continue;
    if (attr.IsRelative()) continue;

    const UString& link = attr.GetPath();
    if (!IsDrivePath(link)) continue;
    // maybe we need to support networks paths also ?

    FString fullPathF;
    if (!NDir::MyGetFullPathName(GetPhyPath(i), fullPathF)) continue;
    UString fullPath = fs2us(fullPathF);
    const UString logPath = GetLogPath(i);
    if (logPath.Len() >= fullPath.Len()) continue;
    if (CompareFileNames(logPath, fullPath.RightPtr(logPath.Len())) != 0) continue;

    const UString prefix = fullPath.Left(fullPath.Len() - logPath.Len());
    if (!IsPathSepar(prefix.Back())) continue;

    unsigned rootPrefixSize = GetRootPrefixSize(prefix);
    if (rootPrefixSize == 0) continue;
    if (rootPrefixSize == prefix.Len()) continue;  // simple case: paths are from root

    if (link.Len() <= prefix.Len()) continue;

    if (CompareFileNames(link.Left(prefix.Len()), prefix) != 0) continue;

    UString newLink = prefix.Left(rootPrefixSize);
    newLink += link.Ptr(prefix.Len());

    CByteBuffer data;
    if (!FillLinkData(data, newLink, attr.IsSymLink())) continue;
...
HRESULT EnumerateDirItemsAndSort(
    NWildcard::CCensor& censor, NWildcard::ECensorPathMode censorPathMode,
    const UString& addPathPrefix, UStringVector& sortedPaths, UStringVector& sortedFullPaths,
    CDirItemsStat& st, IDirItemsCallback* callback) {
  FStringVector paths;

  {
    ...
    FOR_VECTOR(i, dirItems.Items) {
      ...
      if (!dirItem.IsDir()) paths.Add(dirItems.GetPhyPath(i));
    }
  }
...
static void ConvertToLongName(const UString& prefix, UString& name) {
  if (name.IsEmpty() || DoesNameContainWildcard(name)) return;
  NFind::CFileInfo fi;
  const FString path(us2fs(prefix + name));
#  ifndef UNDER_CE
  if (NFile::NName::IsDevicePath(path)) return;
#  endif
  if (fi.Find(path)) name = fs2us(fi.Name);
}

static void ConvertToLongNames(const UString& prefix, CObjectVector<NWildcard::CItem>& items) {
  FOR_VECTOR(i, items) {
    NWildcard::CItem& item = items[i];
    if (item.Recursive || item.PathParts.Size() != 1) continue;
    if (prefix.IsEmpty() && item.IsDriveItem()) continue;
...
static void ConvertToLongNames(const UString& prefix, NWildcard::CCensorNode& node) {
  ...
  for (i = 0; i < node.SubNodes.Size(); i++) {
    ...
    if (prefix.IsEmpty() && NWildcard::IsDriveColonName(name)) continue;
