...
STDMETHODIMP COutStreamWithHash::Write(const void* data, UInt32 size, UInt32* processedSize) {
  ...
  if (_calculate) _hash->Update(data, size);
  _size += size;
...
bool InitLocalPrivileges() {
  ...
  if (!token.OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY | TOKEN_ADJUST_PRIVILEGES))
    return false;
...
static HRESULT Archive_Get_HardLinkNode(
    IInArchive* archive, UInt32 index, CHardLinkNode& h, bool& defined) {
  h.INode = 0;
  ...
  defined = false;
  {
    ...
    if (!ConvertPropVariantToUInt64(prop, h.INode)) return S_OK;
  }
  {
    NCOM::CPropVariant prop;
...

  {
    ...
    for (unsigned i = 1; i < hardIDs.Size(); i++) {
      if (hardIDs[i].Compare(hardIDs[i - 1]) != 0)
        ...
        if (i - 1 != k) hardIDs[k] = hardIDs[i - 1];
        k++;
      }
    }
...
void CArchiveExtractCallback::Init(
    const CExtractNtOptions& ntOptions, const NWildcard::CCensorNode* wildcardCensor,
    const CArc* arc, IFolderArchiveExtractCallback* extractCallback2, bool stdOutMode,
    bool testMode, const FString& directoryPath, const UStringVector& removePathParts,
    bool removePartsForAltStreams, UInt64 packSize) {
  ...
  _extractCallback2.QueryInterface(
      IID_IFolderArchiveExtractCallback2, &_folderArchiveExtractCallback2);
  ...
  if (ExtractToStreamCallback) {
    Int32 useStreams = 0;
    if (ExtractToStreamCallback->UseExtractToStream(&useStreams) != S_OK) useStreams = 0;
    if (useStreams == 0) ExtractToStreamCallback.Release();
  }
...
STDMETHODIMP CArchiveExtractCallback::SetTotal(UInt64 size) {
  ...
  _progressTotal = size;
  _progressTotal_Defined = true;
  if (!_multiArchives && _extractCallback2) return _extractCallback2->SetTotal(size);
  return S_OK;
  COM_TRY_END
}
...
STDMETHODIMP CArchiveExtractCallback::SetCompleted(const UInt64* completeValue) {
  ...
  if (!_extractCallback2) return S_OK;
...
void CArchiveExtractCallback::CreateComplexDirectory(
    const UStringVector& dirPathParts, FString& fullPath) {
  bool isAbsPath = false;

  if (!dirPathParts.IsEmpty()) {
    const UString& s = dirPathParts[0];
    if (s.IsEmpty()) isAbsPath = true;
#if defined(_WIN32) && !defined(UNDER_CE)
    else {
      if (NName::IsDrivePath2(s)) isAbsPath = true;
    }
#endif
  }
  ...
  FOR_VECTOR(i, dirPathParts) {
    if (i != 0) fullPath.Add_PathSepar();
    const UString& s = dirPathParts[i];
    fullPath += us2fs(s);
#if defined(_WIN32) && !defined(UNDER_CE)
    if (_pathMode == NExtract::NPathMode::kAbsPaths)
      if (i == 0 && s.Len() == 2 && NName::IsDrivePath2(s)) continue;
#endif
    CreateDir(fullPath);
  }
}

HRESULT CArchiveExtractCallback::GetTime(
    UInt32 index, PROPID propID, FILETIME& filetime, bool& filetimeIsDefined) {
...
HRESULT CArchiveExtractCallback::SendMessageError_with_LastError(
...
HRESULT CArchiveExtractCallback::SendMessageError2(
...
static UString GetDirPrefixOf(const UString& src) {
  UString s(src);
  if (!s.IsEmpty()) {
    if (IsPathSepar(s.Back())) s.DeleteBack();
...
bool IsSafePath(const UString& path) {
  if (NName::IsAbsolutePath(path)) return false;
...
bool CensorNode_CheckPath2(
    const NWildcard::CCensorNode& node, const CReadArcItem& item, bool& include) {
  bool found = false;
...
#ifdef SUPPORT_ALT_STREAMS
    if (!item.IsAltStream) return true;
#endif
  ...
  if (!item.IsAltStream) return false;

  UStringVector pathParts2 = item.PathParts;
  if (pathParts2.IsEmpty()) pathParts2.AddNew();
  UString& back = pathParts2.Back();
  back += ':';
  back += item.AltStreamName;
  bool include2;

  if (node.CheckPathVect(
          pathParts2,
          true,  // isFile,
          include2)) {
    include = include2;
...
bool CensorNode_CheckPath(const NWildcard::CCensorNode& node, const CReadArcItem& item) {
  bool include;
  if (CensorNode_CheckPath2(node, item, include)) return include;
  return false;
}
...
static FString MakePath_from_2_Parts(const FString& prefix, const FString& path) {
  ...
  if (!path.IsEmpty() && path[0] == ':' && !prefix.IsEmpty() && IsPathSepar(prefix.Back())) {
    if (!NName::IsDriveRootPath_SuperAllowed(prefix)) s.DeleteBack();
  }
#endif
  s += path;
  return s;
...
STDMETHODIMP CArchiveExtractCallback::GetStream(
  ...
  *outStream = NULL;
  ...
  if (_use_baseParentFolder_mode) {
    ...
    if (_pathMode == NExtract::NPathMode::kFullPaths || _pathMode == NExtract::NPathMode::kAbsPaths)
...

  {
    ...
    if (prop.vt != VT_EMPTY) {
      if (prop.vt != VT_UI8) return E_FAIL;
  ...
  */
  ...
  if (linkPath.IsEmpty() && _arc->GetRawProps) {
    ...
    if (dataSize != 0) {
      if (propType != NPropDataType::kRaw) return E_FAIL;
      UString s;
...
#  endif
  ...
  if (!linkPath.IsEmpty() && !isRelative && _removePathParts.Size() != 0) {
    ...
    if (!badPrefix) pathParts.DeleteFrontal(_removePathParts.Size());
  ...
  if (!_ntOptions.AltStreams.Val && _item.IsAltStream) return S_OK;
  ...
  if (_wildcardCensor) {
    if (!CensorNode_CheckPath(*_wildcardCensor, _item)) return S_OK;
  }
    ...
    switch (_pathMode) {
      ...
      case NExtract::NPathMode::kCurPaths: {
        ...
        if (pathParts.Size() < _removePathParts.Size())
          ...
          if (!badPrefix) FOR_VECTOR(i, _removePathParts) {
              if (CompareFileNames(_removePathParts[i], pathParts[i]) != 0) {
                badPrefix = true;
                break;
              }
        ...
        if (badPrefix) {
          if (askExtractMode == NArchive::NExtract::NAskMode::kExtract && !_testMode) return E_FAIL;
        } else
      ...
      case NExtract::NPathMode::kNoPaths: {
        if (!pathParts.IsEmpty()) numRemovePathParts = pathParts.Size() - 1;
        break;
      }
...
#  endif
    ...
    return ExtractToStreamCallback->GetStream7(
        name, BoolToInt(_item.IsDir), outStream, askExtractMode, GetProp);
  }
  ...
  if (askExtractMode == NArchive::NExtract::NAskMode::kExtract && !_testMode) {
    if (_stdOutMode) {
...
#ifdef SUPPORT_ALT_STREAMS
      if (!_item.IsAltStream || !pathParts.IsEmpty() ||
          !(_removePartsForAltStreams || _pathMode == NExtract::NPathMode::kNoPathsAlt))
#endif
        Correct_FsPath(
            _pathMode == NExtract::NPathMode::kAbsPaths, _keepAndReplaceEmptyDirPrefixes, pathParts,
            _item.MainIsDir);
      ...
      if (_item.IsAltStream) {
        ...
        if (needColon) name += (char)(_ntOptions.ReplaceColonForAltStream ? '_' : ':');
        name += s;
      }
      ...
      if (!isAnti) {
        if (!_item.IsDir) {
          if (!pathParts.IsEmpty()) pathParts.DeleteBack();
        }
              ...
              if (NFind::DoesFileExist(fullProcessedPath))
                if (!DeleteFileAlways(fullProcessedPath))
                  if (GetLastError() != ERROR_FILE_NOT_FOUND) {
                    RINOK(
                        SendMessageError_with_LastError(kCantDeleteOutputFile, fullProcessedPath));
                    return S_OK;
                    // return E_FAIL;
                  }
            }
...
#  ifndef UNDER_CE
          ...
          UString relatPath;
          if (isRelative) relatPath = GetDirPrefixOf(_item.Path);
          relatPath += linkPath;
            ...
            if (!existPath.IsEmpty()) {
              if (isHardLink /* || isCopyLink */) {
                ...
                else
                {
                  ...
                  {
                    RINOK(SendMessageError2("Can not find the file for copying", existPath,
                fullProcessedPath));
                  }
                  else
                  {
                    ...
                    {
                      RINOK(SendMessageError2("File size collision for file copying", existPath,
                fullProcessedPath));
                    }
                ...
                if (FillLinkData(data, fs2us(existPath), !isJunction)) {
                  ...
                  if (!attr.Parse(data, data.Size(), errorCode)) {
                    RINOK(SendMessageError(
                        "Internal error for symbolic link file", us2fs(_item.Path)));
                    // return E_FAIL;
                  } else if (!NFile::NIO::SetReparseData(
                                 fullProcessedPath, _item.IsDir, data, (DWORD)data.Size())) {
...
#endif  // SUPPORT_LINKS
        {
          ...
          if (needWriteFile) {
            ...
            if (!_outFileStreamSpec->Open(
                    fullProcessedPath, _isSplit ? OPEN_ALWAYS : CREATE_ALWAYS)) {
            ...
            if (_ntOptions.PreAllocateOutFile && !_isSplit && _curSizeDefined &&
                _curSize > (1 << 12)) {
              ...
              */
              ...
              if (!res) {
                RINOK(SendMessageError_with_LastError(
                    "Can not seek to begin of file", fullProcessedPath));
              }
            }
...
STDMETHODIMP CArchiveExtractCallback::PrepareOperation(Int32 askExtractMode) {
...
#ifndef _SFX
  if (ExtractToStreamCallback) return ExtractToStreamCallback->PrepareOperation7(askExtractMode);
#endif
  ...
  return _extractCallback2->PrepareOperation(
      _item.Path, BoolToInt(_item.IsDir), askExtractMode, _isSplit ? &_position : 0);

  COM_TRY_END
}

HRESULT CArchiveExtractCallback::CloseFile() {
  if (!_outFileStream) return S_OK;
  ...
  if (_fileLengthWasSet && _curSize > processedSize) {
    ...
    _fileLengthWasSet = res;
    if (!res) hres = SendMessageError_with_LastError(kCantSetFileLen, us2fs(_item.Path));
  }
...
#  endif
  ...
  if (!_curSizeDefined) GetUnpackSize();
...
STDMETHODIMP CArchiveExtractCallback::ReportExtractResult(
    UInt32 indexType, UInt32 index, Int32 opRes) {
...
STDMETHODIMP CArchiveExtractCallback::CryptoGetTextPassword(BSTR* password) {
  ...
  if (!_cryptoGetTextPassword) {
    RINOK(_extractCallback2.QueryInterface(IID_ICryptoGetTextPassword, &_cryptoGetTextPassword));
  }
