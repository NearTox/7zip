...
CODER_INTERFACE(ICompressCoder, 0x05) {
  ...
  (ISequentialInStream * inStream, ISequentialOutStream * outStream, const UInt64* inSize,
   const UInt64* outSize, ICompressProgressInfo* progress) PURE;
};
...
/*
  ...
  returns:
    ...
    another error code : some error. For example, it can be error code received from inStream or
  outStream function.
...
CODER_INTERFACE(ICompressSetFinishMode, 0x26) {
  ...
  /* finishMode:
    0 : partial decoding is allowed. It's default mode for ICompressCoder::Code(), if (outSize) is
    defined. 1 : full decoding. The stream must be finished at the end of decoding. */
};
...
CODER_INTERFACE(ICompressSetMemLimit, 0x28) { STDMETHOD(SetMemLimit)(UInt64 memUsage) PURE; };
