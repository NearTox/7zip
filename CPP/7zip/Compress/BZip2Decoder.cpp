...
#define NO_INLINE
    ...
    Byte CDecoder::ReadByte() {
      return (Byte)Base.ReadBits(8);
    }

    UInt32 CBase::ReadBits(unsigned numBits) {
      return BitDecoder.ReadBits(numBits);
    }
    unsigned CBase::ReadBit() {
      return (unsigned)BitDecoder.ReadBits(1);
    }
...
#define RINOK_THREAD(x) { WRes __result_ = (x); if (__result_ != 0) return __result_; }
    ...
    HRESULT CDecoder::CodeResume(ISequentialOutStream *outStream, ICompressProgressInfo *progress) {
      return CodeReal(nullptr, outStream, progress);
    }
    ...
    static THREAD_FUNC_DECL MFThread(void *p) {
      ((CState *)p)->ThreadFunc(); return 0;
    }
    ...
    void CState::ThreadFunc() {
      for(;;) {
        ...
        try {
          ...
          res = e.ErrorCode; if(res == S_OK) res = E_FAIL;
        } catch(...) {
          res = E_FAIL;
        }
        if(res != S_OK) {
