...
CBaseCoder::~CBaseCoder() {
  for (int i = 0; i < BCJ2_NUM_STREAMS + 1; i++) ::MidFree(_bufs[i]);
}
...
HRESULT CBaseCoder::Alloc(bool allocForOrig) {
  ...
  for (unsigned i = 0; i < num; i++) {
    ...
    if (newSize < kMinBufSize) newSize = kMinBufSize;
...
STDMETHODIMP CDecoder::SetInBufSize(UInt32 streamIndex, UInt32 size) {
  _bufsNewSizes[streamIndex] = size;
  return S_OK;
}
STDMETHODIMP CDecoder::SetOutBufSize(UInt32, UInt32 size) {
  _bufsNewSizes[BCJ2_NUM_STREAMS] = size;
  return S_OK;
}
...
void CDecoder::InitCommon() {
  {
    for (int i = 0; i < BCJ2_NUM_STREAMS; i++) dec.lims[i] = dec.bufs[i] = _bufs[i];
  }
...
HRESULT CDecoder::Code(
  ...
  if (numInStreams != BCJ2_NUM_STREAMS || numOutStreams != 1) return E_INVALIDARG;
  ...
  for (;;) {
    if (Bcj2Dec_Decode(&dec) != SZ_OK) return S_FALSE;

    if (dec.state < BCJ2_NUM_STREAMS) {
      size_t totalRead = _extraReadSizes[dec.state];
      {
        Byte* buf = _bufs[dec.state];
        for (size_t i = 0; i < totalRead; i++) buf[i] = dec.bufs[dec.state][i];
      ...
      if (_readRes[dec.state] != S_OK) res = _readRes[dec.state];
    ...
    } else  // if (dec.state <= BCJ2_STATE_ORIG)
    {
      ...
      dec.dest = _bufs[BCJ2_NUM_STREAMS];
      {
        ...
        if (outSizes && outSizes[0]) {
          ...
          if (rem > outSize) rem = (size_t)outSize;
        }
    ...
    if (progress) {
      ...
      if (outSize2 - prevProgress >= (1 << 22)) {
        const UInt64 inSize2 = outSize2 + _inStreamsProcessed[BCJ2_STREAM_RC] -
                               (dec.lims[BCJ2_STREAM_RC] - dec.bufs[BCJ2_STREAM_RC]);
  ...
  if (_finishMode) {
    if (!Bcj2Dec_IsFinished(&dec)) return S_FALSE;

    // we still allow the cases when input streams are larger than required for decoding.
    // so the case (dec.state == BCJ2_STATE_ORIG) is also allowed, if MAIN stream is larger than
    // required.
    if (dec.state != BCJ2_STREAM_MAIN && dec.state != BCJ2_DEC_STATE_ORIG) return S_FALSE;
    ...
    if (inSizes) {
      for (int i = 0; i < BCJ2_NUM_STREAMS; i++) {
        ...
        if (rem != 0)
          return S_FALSE;
        */
        if (inSizes[i] && *inSizes[i] != _inStreamsProcessed[i] - rem) return S_FALSE;
      }
    }
  }
...
STDMETHODIMP CDecoder::Read(void* data, UInt32 size, UInt32* processedSize) {
  ...
  for (;;) {
    ...
    if (sres != SZ_OK) return S_FALSE;

    {
      UInt32 curSize = (UInt32)(dec.dest - (Byte*)data);
      if (curSize != 0) {
        totalProcessed += curSize;
        if (processedSize) *processedSize = totalProcessed;
    ...
    if (dec.state >= BCJ2_NUM_STREAMS) break;

    {
      ...
      if (_readRes[dec.state] != S_OK) return _readRes[dec.state];
      ...
      if (totalRead == 0) {
        if (totalProcessed == 0) res = _readRes[dec.state];
        break;
      }
      ...
      if (BCJ2_IS_32BIT_STREAM(dec.state)) {
        ...
        _extraReadSizes[dec.state] = extraSize;
        if (totalRead < 4) {
          if (totalProcessed != 0) return S_OK;
