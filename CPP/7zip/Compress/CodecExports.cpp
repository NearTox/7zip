...
static void SetPropFromAscii(const char *s, PROPVARIANT *prop) throw()
{
  ...
  BSTR dest = ::SysAllocStringLen(nullptr, len);
  if (dest)
  {
...
{
  if ((value->bstrVal = ::SysAllocStringByteLen((const char *)&guid, sizeof(guid))) != nullptr)
    value->vt = VT_BSTR;
  return S_OK;
}
...
STDAPI GetMethodProperty(UInt32 codecIndex, PROPID propID, PROPVARIANT *value)
{
  ...
  switch (propID)
  {
    ...
    case NMethodPropID::kDecoderIsAssigned:
        value->vt = VT_BOOL;
        value->boolVal = BoolToVARIANT_BOOL(codec.CreateDecoder != nullptr);
      break;
    case NMethodPropID::kEncoderIsAssigned:
        value->vt = VT_BOOL;
        value->boolVal = BoolToVARIANT_BOOL(codec.CreateEncoder != nullptr);
      break;
