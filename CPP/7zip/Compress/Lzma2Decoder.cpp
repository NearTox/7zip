...
CDecoder::~CDecoder() {
  if (_dec) Lzma2DecMt_Destroy(_dec);
}
...
STDMETHODIMP CDecoder::SetDecoderProperties2(const Byte* prop, UInt32 size) {
  if (size != 1) return E_NOTIMPL;
  if (prop[0] > 40) return E_NOTIMPL;
  _prop = prop[0];
  return S_OK;
}
...
#  define LZMA2_DIC_SIZE_FROM_PROP_FULL(p) \
    ((p) == 40 ? 0xFFFFFFFF : (((UInt32)2 | ((p)&1)) << ((p) / 2 + 11)))
...
STDMETHODIMP CDecoder::Code(
...
#ifndef _7ZIP_ST
  {
    ...
    if (_tryMt && numThreads >= 1) {
      ...
      if (expectedBlockSize == expectedBlockSize64 && inBlockMax >= expectedBlockSize) {
        ...
        if (numThreads > okThreads) numThreads = (UInt32)okThreads;
  ...
  */
  ...
  if (props.numThreads > 1) _tryMt = isMT;
#endif
  ...
  if (res == SZ_OK && _finishMode) {
    if (inSize && *inSize != inProcessed) res = SZ_ERROR_DATA;
    if (outSize && *outSize != outWrap.Processed) res = SZ_ERROR_DATA;
  }
...
STDMETHODIMP CDecoder::SetOutStreamSize(const UInt64* outSize) {
  ...
  if (res != SZ_OK) return SResToHRESULT(res);
  return S_OK;
}
...
STDMETHODIMP CDecoder::Read(void* data, UInt32 size, UInt32* processedSize) {
  ...
  _inProcessed += inProcessed;
  if (processedSize) *processedSize = (UInt32)size2;
  if (res != SZ_OK) return SResToHRESULT(res);
  return S_OK;
}
