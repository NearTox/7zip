...
HRESULT CDecoder::CreateInputBuffer() {
  if (!_inBuf || _inBufSizeNew != _inBufSize) {
    ...
    if (!_inBuf) return E_OUTOFMEMORY;
...
HRESULT CDecoder::CodeSpec(
  ...
  if (!_inBuf || !_propsWereSet) return S_FALSE;
  ...
  for (;;) {
    ...
    SRes res = LzmaDec_DecodeToDic(
        &_state, dicPos + size, _inBuf + _inPos, &inProcessed, finishMode, &status);
    ...
    // (_outSizeDefined && _outSize == 0)
    bool outFinished = (_outSizeDefined && _outProcessed >= _outSize);

    bool needStop =
        (res != 0 || (inProcessed == 0 && outProcessed == 0) ||
    ...
    if (needStop || outProcessed >= size) {
      ...
      if (_state.dicPos == _state.dicBufSize) _state.dicPos = 0;
      wrPos = _state.dicPos;
      ...
      if (needStop) {
        ...
        if (status == LZMA_STATUS_FINISHED_WITH_MARK) {
          if (FinishStream)
            if (_outSizeDefined && _outSize != _outProcessed) return S_FALSE;
          return readRes;
        }

        if (outFinished && status != LZMA_STATUS_NEEDS_MORE_INPUT)
          if (!FinishStream || status == LZMA_STATUS_MAYBE_FINISHED_WITHOUT_MARK) return readRes;
...
STDMETHODIMP CDecoder::Code(
    ISequentialInStream* inStream, ISequentialOutStream* outStream, const UInt64* inSize,
    const UInt64* outSize, ICompressProgressInfo* progress) {
  if (!_inBuf) return E_INVALIDARG;
  SetOutStreamSize(outSize);
  HRESULT res = CodeSpec(inStream, outStream, progress);
  if (res == S_OK)
    if (FinishStream && inSize && *inSize != _inProcessed) res = S_FALSE;
  return res;
}
...
STDMETHODIMP CDecoder::Read(void* data, UInt32 size, UInt32* processedSize) {
  ...
  for (;;) {
    ...
    SRes res = LzmaDec_DecodeToBuf(
        &_state, (Byte*)data, &outProcessed, _inBuf + _inPos, &inProcessed, finishMode, &status);
    ...
    if (processedSize) *processedSize += (UInt32)outProcessed;
    ...
    */
    ...
    if (inProcessed == 0 && outProcessed == 0) return readRes;
  }
}
...
HRESULT CDecoder::ReadFromInputStream(void* data, UInt32 size, UInt32* processedSize) {
  ...
  while (size != 0) {
    if (_inPos == _inLim) {
      _inPos = _inLim = 0;
      if (readRes == S_OK) readRes = _inStream->Read(_inBuf, _inBufSize, &_inLim);
      if (_inLim == 0) break;
    }
    ...
    size -= cur;
    data = (Byte*)data + cur;
    if (processedSize) *processedSize += cur;
  }
