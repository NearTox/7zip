...
HRESULT CDecoder::Decode(
    ISequentialInStream* seqInStream, ISequentialOutStream* outStream, const UInt64* outSizeLimit,
    bool finishStream, ICompressProgressInfo* progress) {
  MainDecodeSRes = S_OK;
...
#ifndef _7ZIP_ST
  {
    ...
    if (_tryMt && numThreads > 1) {
      ...
      if (memUsage != _memUsage) memUsage = (size_t)0 - 1;
...
// _tryMt = isMT;
  ...
  if (res == SZ_OK && finishStream) {
    ...
    if (inSize && *inSize != Stat.PhySize)
      res = SZ_ERROR_DATA;
    */
    if (outSizeLimit && *outSizeLimit != outWrap.Processed) res = SZ_ERROR_DATA;
  }

  return SResToHRESULT_Code(res);
}

HRESULT CComDecoder::Code(
