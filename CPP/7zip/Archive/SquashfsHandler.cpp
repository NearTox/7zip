...
HRESULT CHandler::Decompress(ISequentialOutStream *outStream, Byte *outBuf, bool *outBufWasWritten, UInt32 *outBufWasWrittenSize, UInt32 inSize, UInt32 outSizeMax)
{
  ...
  if (_h.SeveralMethods)
  {
    ...
    RINOK(_stream->Seek(-1, STREAM_SEEK_CUR, nullptr));
  ...
  if (method == kMethod_ZLIB)
  {
    ...
    RINOK(_zlibDecoder->Code(_limitedInStream, outStream, nullptr, nullptr, nullptr));
  ...
  else if (method == kMethod_LZMA)
  {
    ...
    RINOK(_lzmaDecoder->Code(_limitedInStream, outStream, nullptr, &outSize, nullptr));
...
HRESULT CHandler::ReadMetadataBlock(UInt32 &packSize)
{
  ...
  if (isCompressed)
  {
    ...
    RINOK(Decompress(_dynOutStream, nullptr, nullptr, nullptr, size, kMetadataBlockSize));
  }
  else
  {
    // size != 0 here
...
HRESULT CHandler::ReadData(CData &data, UInt64 start, UInt64 end)
{
  ...
  RINOK(_stream->Seek(start, STREAM_SEEK_SET, nullptr));
  ...
  while (rem != 0)
  {
    ...
    for (UInt32 i = 0; i < count; i++)
    {
      ...
      if (_openCallback)
      {
        ...
        {
          RINOK(_openCallback->SetCompleted(&numFiles, nullptr));
        }
      }
...
HRESULT CHandler::Open2(IInStream *inStream)
{
  ...
  if (_h.NumFrags != 0)
  {
    ...
    RINOK(inStream->Seek(_h.FragTable, STREAM_SEEK_SET, nullptr));
    ...
    for (UInt32 i = 0; i < numBlocks; i++)
    {
      ...
      RINOK(_stream->Seek(offset, STREAM_SEEK_SET, nullptr));
      _dynOutStreamSpec->Init();
  ...
  // RINOK(inStream->Seek(_h.InodeTable, STREAM_SEEK_SET, nullptr));
  ...
  else
  {
    ...
    RINOK(inStream->Seek(_h.UidTable, STREAM_SEEK_SET, nullptr));
    RINOK(ReadStream_FALSE(inStream, data, numBlocksBytes));
    ...
    for (UInt32 i = 0; i < numBlocks; i++)
    {
      ...
      RINOK(_stream->Seek(offset, STREAM_SEEK_SET, nullptr));
      RINOK(ReadMetadataBlock(nullptr, _uids + kMetadataBlockSize * i, packSize, unpackSize));
      if (unpackSize != kMetadataBlockSize)
  ...
  */

  {
    ...
    Byte buf[alignSize];
    RINOK(inStream->Seek(_h.Size, STREAM_SEEK_SET, nullptr));
...
HRESULT CHandler::ReadBlock(UInt64 blockIndex, Byte *dest, size_t blockSize)
{
  ...
  {
    ClearCache();
    RINOK(_stream->Seek(blockOffset, STREAM_SEEK_SET, nullptr));
  ...
  for (i = 0; i < numItems; i++)
  {
    ...
    int res = NExtract::NOperationResult::kDataError;
    {
      ...
      else
      {
        RINOK(hres);
        {
          hres = copyCoder->Code(inSeqStream, outStream, nullptr, nullptr, progress);
          if (hres == S_OK)
          {
