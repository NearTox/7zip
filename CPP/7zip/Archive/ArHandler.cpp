...
HRESULT CHandler::ParseLongNames(IInStream *stream)
{
  ...
  if (item.Size > ((UInt32)1 << 30))
    return S_FALSE;
  RINOK(stream->Seek(item.GetDataPos(), STREAM_SEEK_SET, nullptr));
    ...
    IArchiveOpenCallback *callback)
{
  COM_TRY_BEGIN
  {
    ...
    if (callback)
    {
      RINOK(callback->SetTotal(nullptr, &fileSize));
    ...
    Int32 testMode, IArchiveExtractCallback *extractCallback)
{
  ...
  for (i = 0; i < numItems; i++)
  {
    ...
    {
      RINOK(_stream->Seek(item.GetDataPos(), STREAM_SEEK_SET, nullptr));
