    ...
    IArchiveUpdateCallback *updateCallback, UInt64 &totalComplexity)
{
  ...
  UInt64 size;
  // UInt32 attrib;
  if (getProps->GetProps(&size, &cTime, &aTime, &mTime, nullptr) != S_OK)
    return;
    ...
    IArchiveUpdateCallback *updateCallback)
{
  ...
  COneMethodInfo *oneMethodMain = nullptr;
  if (!options2._methods.IsEmpty())
  ...
  RINOK(mtCompressProgressMixer.SetRatioInfo(0, nullptr, nullptr));

  archive.WriteCentralDir(items, comment);

  complexity += kCentralHeaderSize * updateItems.Size() + 1;
  mtProgressMixerSpec->Mixer2->SetProgressOffset(complexity);
  return mtCompressProgressMixer.SetRatioInfo(0, nullptr, nullptr);
    ...
    IArchiveUpdateCallback *updateCallback)
{
  ...
  bool outSeqMode;
  {
    ...
    if (inArchive)
    {
      if (!inArchive->IsMultiVol && inArchive->ArcInfo.Base > 0 && !removeSfx)
      {
        ...
        RINOK(baseStream->Seek(0, STREAM_SEEK_SET, nullptr));
        RINOK(NCompress::CopyStream_ExactSize(baseStream, seqOutStream, inArchive->ArcInfo.Base, nullptr));
      }
    }
    ...
    outSeqMode = (outStreamReal == nullptr);
  }
  ...
  if (inArchive)
  {
    ...
    {
      IInStream *baseStream = inArchive->GetBaseStream();
      RINOK(baseStream->Seek(inArchive->ArcInfo.Base, STREAM_SEEK_SET, nullptr));
      UInt64 embStubSize = inArchive->ArcInfo.MarkerPos2 - inArchive->ArcInfo.Base;
      RINOK(NCompress::CopyStream_ExactSize(baseStream, outStream, embStubSize, nullptr));
  ...
  return Update2(
      ...
      inArchive ? &inArchive->ArcInfo.Comment : nullptr,
      updateCallback);
}
