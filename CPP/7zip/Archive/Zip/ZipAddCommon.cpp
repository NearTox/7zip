...
CAddCommon::CAddCommon(const CCompressionMethodMode &options):
    _options(options),
    _copyCoderSpec(nullptr),
    _cryptoStreamSpec(nullptr),
    _buf(nullptr),
    _isLzmaEos(false)
    {}
    ...
    ICompressProgressInfo *progress, CCompressingResult &opRes)
{
  ...
  for (unsigned i = 0; i < numTestMethods; i++)
  {
    ...
    if (i != 0)
    {
      ...
      {
        inSecCrcStreamSpec->Init();
        RINOK(inStream2->Seek(0, STREAM_SEEK_SET, nullptr));
      }

      RINOK(outStream->SetSize(0));
      RINOK(outStream->Seek(0, STREAM_SEEK_SET, nullptr));
    }
    ...
    switch (method)
    {
      case NCompressionMethod::kStore:
      {
        ...
        RINOK(_copyCoder->Code(inCrcStream, outStreamNew, nullptr, nullptr, progress));
        break;
      }
      ...
      default:
      {
        if (!_compressEncoder)
        {
          CLzmaEncoder *_lzmaEncoder = nullptr;
            ...
            if (setCoderProps)
            {
              if (!_options._methods.IsEmpty())
              {
                ...
                RINOK(oneMethodMain->SetCoderProps(setCoderProps,
                    _options._dataSizeReduceDefined ? &_options._dataSizeReduce : nullptr));
              }
            }
          }
          if (method == NCompressionMethod::kLZMA)
        ...
        RINOK(_compressEncoder->Code(inCrcStream, outStreamNew, nullptr, nullptr, progress));
        break;
      }
    }
