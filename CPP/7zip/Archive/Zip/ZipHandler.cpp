    ...
    const UInt64 *inSize, const UInt64 *outSize, ICompressProgressInfo *progress)
{
  ...
  return Decoder->Code(inStream, outStream, inSize ? &inSize2 : nullptr, outSize, progress);
}
    ...
    Int32 &res)
{
  ...
  if (item.IsEncrypted())
  {
    ...
    else
    {
      res = NExtract::NOperationResult::kWrongPassword;
      return S_OK;
      // RINOK(cryptoSetPassword->CryptoSetPassword(nullptr, 0));
    }
  }
...

  {
    ...
    if (item.IsEncrypted())
    {
      ...
      if (result == S_OK)
      {
        ...
        // RINOK(filterStreamSpec->SetOutStreamSize(nullptr));
    ...
    if (result == S_OK)
    {
      ...
      result = coder->Code(inStreamNew, outStream,
          isFullStreamExpected ? &coderPackSize : nullptr,
          // nullptr,
          useUnpackLimit ? &item.Size : nullptr,
          compressProgress);
