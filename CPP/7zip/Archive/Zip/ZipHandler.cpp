...
namespace NArchive {
  namespace NZip {
    ...
    HRESULT CLzmaDecoder::Code(ISequentialInStream *inStream, ISequentialOutStream *outStream,
      ...
      return Decoder->Code(inStream, outStream, nullptr, outSize, progress);
    }
    ...
    HRESULT CZipDecoder::Decode(
      ...
      if(m == methodItems.Size()) {
        ...
        if(id == NFileHeader::NCompressionMethod::kStored)
          ...
          RINOK(CreateCoder(szMethodID, false, mi.Coder));
...

      {
        ...
        if(result == S_OK)
          result = coder->Code(inStreamNew, outStream, nullptr, &item.Size, compressProgress);
