...
namespace NArchive {
  namespace NZip {
    ...
    HRESULT CInArchive::ReadBytes(void *data, UInt32 size, UInt32 *processedSize) {
      ...
      if(_inBufMode) {
        try {
          realProcessedSize = _inBuffer.ReadBytes((Byte *)data, size);
    ...
    void CInArchive::SafeReadBytes(void *data, unsigned size) {
      ...
      if(!_inBufMode)
        ...
        for(;;) {
          ...
          // if (Vols.NeedSeek)
          {
            result = s.Stream->Seek(0, STREAM_SEEK_SET, nullptr);
            m_Position = 0;
            if(result != S_OK)
              break;
            Vols.NeedSeek = false;
    ...
    UInt16 CInArchive::ReadUInt16() {
      Byte buf[2]; SafeReadBytes(buf, 2); return Get16(buf);
    }
    UInt32 CInArchive::ReadUInt32() {
      Byte buf[4]; SafeReadBytes(buf, 4); return Get32(buf);
    }
    UInt64 CInArchive::ReadUInt64() {
      Byte buf[8]; SafeReadBytes(buf, 8); return Get64(buf);
    }
    ...
    HRESULT CInArchive::ReadLocalItemAfterCdItem(CItemEx &item, bool &isAvail) {
      ...
      try {
        ...
        if(IsMultiVol) {
          ...
          RINOK(str2->Seek(offset, STREAM_SEEK_SET, nullptr));
          Stream = str2;
    ...
    HRESULT CInArchive::TryReadCd(CObjectVector<CItemEx> &items, const CCdInfo &cdInfo, UInt64 cdOffset, UInt64 cdSize) {
      ...
      if(!IsMultiVol) {
        ...
        if(!str2)
          return S_FALSE;
        RINOK(str2->Seek(cdOffset, STREAM_SEEK_SET, nullptr));
        stream = str2;
        Vols.NeedSeek = false;
    ...
    HRESULT CInArchive::ReadCd(CObjectVector<CItemEx> &items, UInt32 &cdDisk, UInt64 &cdOffset, UInt64 &cdSize) {
      ...
      if(Callback) {
        RINOK(Callback->SetTotal(&cdInfo.NumEntries, nullptr));
      }
    ...
    HRESULT CVols::Read(void *data, UInt32 size, UInt32 *processedSize) {
      ...
      for(;;) {
        ...
        if(!s.Stream)
          return S_FALSE;
        if(NeedSeek) {
          RINOK(s.Stream->Seek(0, STREAM_SEEK_SET, nullptr));
          NeedSeek = false;
        }
...
#define COPY_ECD_ITEM_32(n) if (!isZip64 || ecd. n != 0xFFFFFFFF) ecd64. n = ecd. n;
    ...
    HRESULT CInArchive::ReadHeaders2(CObjectVector<CItemEx> &items) {
      ...
      if(!MarkerIsFound) {
        ...
        RINOK(Stream->Seek(m_Position, STREAM_SEEK_SET, nullptr));
      ...
      if(IsMultiVol && Vols.StartVolIndex != 0) {
        ...
        if(IsMultiVol && (unsigned)Vols.StartVolIndex < Vols.Streams.Size()) {
          ...
          if(!Stream)
            ...
            RINOK(Stream->Seek(curPos, STREAM_SEEK_SET, nullptr));
            m_Position = curPos;
          }
        } else
          IsMultiVol = false;
    ...
    HRESULT CInArchive::GetItemStream(const CItemEx &item, bool seekPackData, CMyComPtr<ISequentialInStream> &stream) {
      ...
      if(!IsMultiVol) {
        if(UseDisk_in_SingleVol && item.Disk != EcdVolIndex)
          return S_OK;
        pos += ArcInfo.Base;
        RINOK(StreamRef->Seek(pos, STREAM_SEEK_SET, nullptr));
        stream = StreamRef;
        return S_OK;
      }
      ...
      if(!str2)
        return S_OK;
      RINOK(str2->Seek(pos, STREAM_SEEK_SET, nullptr));
