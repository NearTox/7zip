...
namespace NArchive {
  namespace NCab {
    ...
    STDMETHODIMP CHandler::Open(IInStream *inStream,
      ...
      // try
      {
        while(nextStream) {
          ...
          RINOK(callback->SetCompleted(&numItems, nullptr));

          nextStream = nullptr;

          for(;;) {
            const COtherArc *otherArc = nullptr;
    ...
    public:
      ...
      CFolderOutStream() : TempBuf(nullptr) {}
      ~CFolderOutStream() {
        FreeTempBuf();
      }
      void Init(
      ...
      bool NeedMoreWrite() const {
        return (m_FolderSize > m_PosInFolder);
      }
      UInt64 GetRemain() const {
        return m_FolderSize - m_PosInFolder;
      }
      UInt64 GetPosInFolder() const {
    ...
    STDMETHODIMP CHandler::Extract(const UInt32 *indices, UInt32 numItems,
      ...
      NCompress::NDeflate::NDecoder::CCOMCoder *deflateDecoderSpec = nullptr;
      CMyComPtr<ICompressCoder> deflateDecoder;

      NCompress::NLzx::CDecoder *lzxDecoderSpec = nullptr;
      CMyComPtr<IUnknown> lzxDecoder;

      NCompress::NQuantum::CDecoder *quantumDecoderSpec = nullptr;
...

        {
          ...
          for(UInt32 bl = 0; cabFolderOutStream->NeedMoreWrite();) {
            ...
            if(bl == 0) {
              ...
              RINOK(db2.Stream->Seek(db2.StartPosition + folder2.DataStart, STREAM_SEEK_SET, nullptr));
            }
            ...
            switch(folder2.GetMethod()) {
              case NHeader::NMethod::kNone:
                res = copyCoder->Code(cabBlockInStream, outStream, nullptr, &unpackSize64, nullptr);
                break;
              ...
              case NHeader::NMethod::kMSZip:
                ...
                res = deflateDecoder->Code(cabBlockInStream, outStream, nullptr, &unpackSize64, nullptr);
                if(res == S_OK) {
