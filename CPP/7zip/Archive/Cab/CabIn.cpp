...
namespace NArchive {
  namespace NCab {
    ...
    HRESULT CInArchive::Open2(CDatabaseEx &db, const UInt64 *searchHeaderSizeLimit) {
      ...
      CLimitedSequentialInStream *limitedStreamSpec = nullptr;
      CMyComPtr<ISequentialInStream> limitedStream;

      // for (int iii = 0; iii < 10000; iii++)
      {
        // db.StartPosition = temp; RINOK(db.Stream->Seek(db.StartPosition, STREAM_SEEK_SET, nullptr));
      ...
      if(_inBuffer.GetProcessedSize() - startInBuf != ai.FileHeadersOffset) {
        // printf("\n!!! Seek Error !!!!\n");
        // fflush(stdout);
        RINOK(db.Stream->Seek(db.StartPosition + ai.FileHeadersOffset, STREAM_SEEK_SET, nullptr));
    ...
    HRESULT CInArchive::Open(CDatabaseEx &db, const UInt64 *searchHeaderSizeLimit) {
      try {
        ...
        return e.ErrorCode;
      } catch(CUnexpectedEndException &) {
        UnexpectedEnd = true; return S_FALSE;
      }
    }
