...
IMP_IInArchive_Props IMP_IInArchive_ArcProps
    ...
    STDMETHODIMP
    CHandler::GetArchiveProperty(PROPID propID, PROPVARIANT* value) {
  NCOM::CPropVariant prop;
  switch (propID) {
    case kpidMainSubfile: prop = (UInt32)0; break;
    case kpidPhySize:
      if (!_sizes.IsEmpty()) prop = _sizes[0];
      break;
...
HRESULT CHandler::Open2(IInStream* stream, IArchiveOpenCallback* callback) {
  ...
  if (!volumeCallback) return S_FALSE;

  UString name;
  {
    NCOM::CPropVariant prop;
    RINOK(volumeCallback->GetProperty(kpidName, &prop));
    if (prop.vt != VT_BSTR) return S_FALSE;
    name = prop.bstrVal;
  }
  ...
  if (ext2.Len() >= 2 && StringsAreEqual_Ascii(ext2.RightPtr(2), "aa")) {
    ...
    while (numLetters < ext2.Len()) {
      if (ext2[ext2.Len() - numLetters - 1] != 'a') break;
      numLetters++;
    }
  } else if (ext.Len() >= 2 && StringsAreEqual_Ascii(ext2.RightPtr(2), "01")) {
    while (numLetters < ext2.Len()) {
      if (ext2[ext2.Len() - numLetters - 1] != '0') break;
      numLetters++;
    }
    if (numLetters != ext.Len()) return S_FALSE;
  } else
    return S_FALSE;
  ...
  for (;;) {
    UString fullName;
    if (!seqName.GetNextName(fullName)) break;
...
STDMETHODIMP CHandler::Extract(
  ...
  if (!testMode && !outStream) return S_OK;
