...
{
  return InStream->Seek(cluster << Header.ClusterSizeLog, STREAM_SEEK_SET, nullptr);
}
...
HRESULT CDatabase::Open()
{
  ...
  */

  {
    ...
    {
      RINOK(InStream->Seek(Header.NumSectors << Header.SectorSizeLog, STREAM_SEEK_SET, nullptr));
  ...
  CMyComPtr<IInStream> mftStream;
  {
    ...
    if (!mftRec.Parse(ByteBuf, Header.SectorSizeLog, numSectorsInRec, 0, nullptr))
      return S_FALSE;
    if (!mftRec.IsFILE())
      return S_FALSE;
  ...
  for (UInt64 pos64 = 0;;)
  {
    ...
    for (size_t i = 0; readSize >= recSize; i += recSize, readSize -= recSize)
    {
      ...
      CObjectVector<CAttr> *attrs = nullptr;
...
static const CStatProp kProps[] =
{
  ...
  { nullptr, kpidPackSize, VT_UI8},

  // { nullptr, kpidLink, VT_BSTR},

  // { "Link 2", kpidLink2, VT_BSTR},
  // { "Link Type", kpidLinkType, VT_UI2},
  { nullptr, kpidINode, VT_UI8},

  { nullptr, kpidMTime, VT_FILETIME},
  { nullptr, kpidCTime, VT_FILETIME},
  { nullptr, kpidATime, VT_FILETIME},
  ...
  { nullptr, kpidAttrib, VT_UI4},
  { nullptr, kpidNumBlocks, VT_UI4},
  { nullptr, kpidIsDeleted, VT_BOOL},
};
*/
...
{
  { nullptr, kpidVolumeName, VT_BSTR},
  { nullptr, kpidFileSystem, VT_BSTR},
  { nullptr, kpidClusterSize, VT_UI4},
  { nullptr, kpidSectorSize, VT_UI4},
  { "Record Size", kpidRecordSize, VT_UI4},
  { nullptr, kpidHeadersSize, VT_UI8},
  { nullptr, kpidCTime, VT_FILETIME},
  { nullptr, kpidId, VT_UI8},
};
...
STDMETHODIMP CHandler::GetArchiveProperty(PROPID propID, PROPVARIANT *value)
{
  ...
  const CMftRec *volRec = (Recs.Size() > kRecIndex_Volume ? &Recs[kRecIndex_Volume] : nullptr);
  ...
  for (i = 0; i < numItems; i++)
  {
    ...
    int res = NExtract::NOperationResult::kDataError;
    {
      ...
      else
      {
        ...
        if (inStream)
        {
          hres = copyCoder->Code(inStream, outStream, nullptr, nullptr, progress);
