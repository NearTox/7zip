...
class CHandler: public CHandlerImg
{
  ...
  void AddErrorMessage(const char *message, const wchar_t *name = nullptr)
  {
  ...
  {
    return Open2(stream, nullptr, openArchiveCallback, 0);
  }
  void CloseAtError();
...
HRESULT CHandler::Seek(UInt64 offset) { return Stream->Seek(_startOffset + offset, STREAM_SEEK_SET, nullptr); }
...
HRESULT CHandler::Open3()
{
  ...
  RINOK(Stream->Seek(fileSize - kHeaderSize, STREAM_SEEK_SET, nullptr));
...
STDMETHODIMP CHandler::Read(void *data, UInt32 size, UInt32 *processedSize)
{
  ...
  if (blockSectIndex == kUnusedBlock)
  {
    if (ParentStream)
    {
      RINOK(ParentStream->Seek(_virtPos, STREAM_SEEK_SET, nullptr));
  ...
  else
  {
    ...
    for (UInt32 cur = 0; cur < size;)
    {
      ...
      if (((BitMap[bmi >> 3] >> (7 - (bmi & 7))) & 1) == 0)
      {
        if (ParentStream)
        {
          RINOK(ParentStream->Seek(_virtPos + cur, STREAM_SEEK_SET, nullptr));
...
static const CStatProp kArcProps[] =
{
  ...
  { nullptr, kpidClusterSize, VT_UI8},
  { nullptr, kpidMethod, VT_BSTR},
  { "Parent", kpidParent, VT_BSTR},
  { nullptr, kpidCreatorApp, VT_BSTR},
