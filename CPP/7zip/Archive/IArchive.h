...
IArchiveExtractCallback::GetStream()
  ...
  Out:
      (*inStream == nullptr) - for directories
      (*inStream == nullptr) - if link (hard link or symbolic link) was created
      if (*inStream == nullptr && askMode == NExtract::NAskMode::kExtract)
      {
...
IInArchive::Open
    ...
    if (maxCheckStartPosition == nullptr), the handler can try to search archive start in stream
...
GetStream out:
  Result:
    S_OK:
      (*inStream == nullptr) - only for directories
                          - the bug was fixed in 9.33: (*Stream == nullptr) was in case of anti-file
      (*inStream != nullptr) - for any file, even for empty file or anti-file
