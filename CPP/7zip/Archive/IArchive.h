...
enum EEnum { kWindows, kUnix, kDOS };
}
...
enum { kNoIndex = 0, kInArcIndex, kBlockIndex, kOutArcIndex };
}
...
#define INTERFACE_IArchiveExtractCallback(x)                                    \
  INTERFACE_IProgress(x) STDMETHOD(GetStream)(                                  \
      UInt32 index, ISequentialOutStream * *outStream, Int32 askExtractMode) x; \
...
ARCHIVE_INTERFACE(IArchiveOpenSetSubArchiveName, 0x50) {
  ...
  Notes:
    Don't call IInArchive functions for same IInArchive object from different threads
  simultaneously. Some IInArchive handlers will work incorrectly in that case.
  */
...
#define INTERFACE_IInArchive(x)                                                                 \
  STDMETHOD(Open)                                                                               \
  (IInStream * stream, const UInt64* maxCheckStartPosition, IArchiveOpenCallback* openCallback) \
  ...
  (UInt32 index, PROPID propID, PROPVARIANT * value) MY_NO_THROW_DECL_ONLY x;                   \
  STDMETHOD(Extract)                                                                            \
  (const UInt32* indices, UInt32 numItems, Int32 testMode,                                      \
   IArchiveExtractCallback* extractCallback) MY_NO_THROW_DECL_ONLY x;                           \
...
#define PROP_DATA_TYPE_wchar_t_PTR_Z_LE \
  ((NPropDataType::kMask_Utf | NPropDataType::kMask_ZeroEnd) + (sizeof(wchar_t) >> 1))
...
ARCHIVE_INTERFACE(IArchiveOpenSeq, 0x61) { STDMETHOD(OpenSeq)(ISequentialInStream * stream) PURE; };
  ...
  */
  ...
  {
    STDMETHOD(ArcOpen2)(ISequentialInStream *stream, UInt32 flags, IArchiveOpenCallback
  *openCallback) PURE;
  };
  */
...
#define INTERFACE_IArchiveUpdateCallback2(x)                                                   \
  INTERFACE_IArchiveUpdateCallback(x) STDMETHOD(GetVolumeSize)(UInt32 index, UInt64 * size) x; \
...
#define INTERFACE_IOutArchive(x)                                                                  \
  ...
  (ISequentialOutStream * outStream, UInt32 numItems, IArchiveUpdateCallback * updateCallback) x; \
...
ARCHIVE_INTERFACE(ISetProperties, 0x03) {
  ...
  (const wchar_t* const* names, const PROPVARIANT* values, UInt32 numProps) PURE;
};
...
/* Exe handler: the handler for executable format (PE, ELF, Mach-O).
   SFX archive: executable stub + some tail data.
     ...
     for 9.31+: exe handler parses SFX archives as executable format, only if AllowTail(1) was
   called */
...
#define IMP_IInArchive_ArcProps_NO                                            \
  IMP_IInArchive_ArcProps_NO_Table STDMETHODIMP CHandler::GetArchiveProperty( \
      PROPID, PROPVARIANT* value) {                                           \
    value->vt = VT_EMPTY;                                                     \
    return S_OK;                                                              \
  }
