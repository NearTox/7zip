...
static const char * const g_FeatureCompat_Flags[] =
{
  ...
  , nullptr // { 7, "EXCLUDE_INODE" // not used
  , nullptr // { 8, "EXCLUDE_BITMAP" // not in kernel
  , "SPARSE_SUPER2"
};
...
HRESULT CHandler::SeekAndRead(IInStream *inStream, UInt64 block, Byte *data, size_t size)
{
  ...
  if (((size + ((size_t)1 << _h.BlockBits) - 1) >> _h.BlockBits) > _h.NumBlocks - block)
    return S_FALSE;
  RINOK(inStream->Seek((UInt64)block << _h.BlockBits, STREAM_SEEK_SET, nullptr));
  _totalRead += size;
...
HRESULT CHandler::Open2(IInStream *inStream)
{
...

  {
    ...
    if (_openCallback)
    {
      RINOK(_openCallback->SetTotal(nullptr, &_phySize));
    }
...
STDMETHODIMP CExtInStream::Read(void *data, UInt32 size, UInt32 *processedSize)
{
...

  {
    ...
    if (phy != _phyPos)
    {
      RINOK(Stream->Seek(phy, STREAM_SEEK_SET, nullptr));
      _phyPos = phy;
    }
