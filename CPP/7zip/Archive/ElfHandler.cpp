...
static const CStatProp kProps[] =
{
  ...
  { nullptr, kpidVirtualSize, VT_UI8 },
  { nullptr, kpidOffset, VT_UI8 },
  { nullptr, kpidVa, VT_UI8 },
...
HRESULT CHandler::Open2(IInStream *stream)
{
  ...
  if (_header.NumSegments != 0)
  {
    ...
    RINOK(stream->Seek(_header.ProgOffset, STREAM_SEEK_SET, nullptr));
  ...
  if (_header.NumSections != 0)
  {
    ...
    RINOK(stream->Seek(_header.SectOffset, STREAM_SEEK_SET, nullptr));
  ...
  if (addSections)
  {
    if (_header.NamesSectIndex < _sections.Size())
    {
        ...
        && (Int64)sect.Offset >= 0)
      {
        ...
        RINOK(stream->Seek(sect.Offset, STREAM_SEEK_SET, nullptr));
        RINOK(ReadStream_FALSE(stream, _namesData, (size_t)size));
      }
    }

    /*
    // we will not delete nullptr sections, since we have links to section via indexes
    ...
    Int32 testMode, IArchiveExtractCallback *extractCallback)
{
  ...
  for (i = 0; i < numItems; i++, currentTotalSize += currentItemSize)
  {
    ...
    RINOK(_inStream->Seek(offset, STREAM_SEEK_SET, nullptr));
