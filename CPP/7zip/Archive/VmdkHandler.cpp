      ...
      if (table.Size() != 0)
      {
        ...
        if (v != 0 && v != extent.ZeroSector)
        {
          ...
          if (extent.NeedDeflate)
          {
            ...
            HRESULT res = _zlibDecoderSpec->Code(_bufInStream, _bufOutStream, nullptr, &blockSize64, nullptr);
...
STDMETHODIMP CHandler::GetArchiveProperty(PROPID propID, PROPVARIANT *value)
{
  ...
  const CExtent *e = nullptr;
  const CDescriptor *desc = nullptr;
...
{
  sector <<= 9;
  RINOK(stream->Seek(sector, STREAM_SEEK_SET, nullptr));
  ...
  if (memcmp(buf, k_Signature, sizeof(k_Signature)) != 0)
  {
    ...
    */
    ...
    if (_descriptor.Extents.Size() > 1)
    {
      ...
      RINOK(openCallback->SetTotal(&numFiles, nullptr));
    }
  }
    ...
    unsigned numVols, unsigned volIndex, UInt64 &complexity)
{
  ...
  if (h.gdOffset == (UInt64)(Int64)-1)
  {
    ...
    if (endPos < kEndSize)
      return S_FALSE;
    RINOK(stream->Seek(endPos - kEndSize, STREAM_SEEK_SET, nullptr));
  ...
  if (openCallback)
  {
    complexity += (UInt64)numGdeEntries << (k_NumMidBits + 2);
    {
      ...
      RINOK(openCallback->SetTotal((numVols == 1) ? nullptr : &numVols2, &complexity));
    }
    if (numVols != 1)
    {
      const UInt64 volIndex2 = volIndex;
      RINOK(openCallback->SetCompleted(numVols == 1 ? nullptr : &volIndex2, &complexityStart));
    }
  }
  ...
  for (size_t i = 0; i < numGdeEntries; i++)
  {
...

    {
      ...
      if (openCallback && (i - numProcessed_Prev) >= 1024)
      {
        ...
        const UInt64 volIndex2 = volIndex;
        RINOK(openCallback->SetCompleted(numVols == 1 ? nullptr : &volIndex2, &comp));
        numProcessed_Prev = i;
      }
