...
{
  // { nullptr, kpidWarning, VT_BSTR},
  { nullptr, kpidCpu, VT_BSTR},
  { nullptr, kpidBit64, VT_BOOL},
  { nullptr, kpidCharacts, VT_BSTR},
  { nullptr, kpidCTime, VT_FILETIME},
  { nullptr, kpidHeadersSize, VT_UI4},
  { nullptr, kpidChecksum, VT_UI4},
  { nullptr, kpidName, VT_BSTR},
...
HRESULT CHandler::LoadDebugSections(IInStream *stream, bool &thereIsSection)
{
  ...
  RINOK(stream->Seek(pa, STREAM_SEEK_SET, nullptr));
...
HRESULT CHandler::OpenResources(unsigned sectionIndex, IInStream *stream, IArchiveOpenCallback *callback)
{
  ...
  size_t fileSize = sect.PSize;
  {
...

    {
      ...
      if (callback)
        RINOK(callback->SetTotal(nullptr, &fileSize64));
    }

    RINOK(stream->Seek(sect.Pa, STREAM_SEEK_SET, nullptr));
    ...
    {
      {
        ...
        if (callback)
          RINOK(callback->SetCompleted(nullptr, &offset64))
      }
...
HRESULT CHandler::Open2(IInStream *stream, IArchiveOpenCallback *callback)
{
  ...
  else
  {
    ...
    }
    {
      ...
      RINOK(stream->Seek(_peOffset, STREAM_SEEK_SET, nullptr));
  ...
  if (thereISDebug)
  {
    ...
    if (alignPos != 0)
    {
      ...
      RINOK(stream->Seek(_totalSize, STREAM_SEEK_SET, nullptr));
      buffer.Alloc(kAlign);
      Byte *buf = buffer;
  ...
  if (_header.NumSymbols > 0 && _header.PointerToSymbolTable >= optStart)
  {
    ...
    RINOK(stream->Seek((UInt64)_header.PointerToSymbolTable + size, STREAM_SEEK_SET, nullptr));
    Byte buf[4];
    ...
    Int32 testMode, IArchiveExtractCallback *extractCallback)
{
  ...
  for (i = 0; i < numItems; i++, currentTotalSize += currentItemSize)
  {
    ...
    else
    {
      ...
      RINOK(extractCallback->PrepareOperation(askMode));
      RINOK(_stream->Seek(sect.Pa, STREAM_SEEK_SET, nullptr));
    ...
    Int32 testMode, IArchiveExtractCallback *extractCallback)
{
  ...
  for (i = 0; i < numItems; i++)
  {
    ...
    RINOK(_stream->Seek(item.Pa, STREAM_SEEK_SET, nullptr));
    streamSpec->Init(item.PSize);
    RINOK(copyCoder->Code(inStream, realOutStream, nullptr, nullptr, progress));
