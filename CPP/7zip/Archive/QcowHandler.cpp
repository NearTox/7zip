      ...
      if (buffer.Size() != 0)
      {
        ...
        if (v != 0)
        {
          if ((v & _compressedFlag) != 0)
          {
            ...
            HRESULT res = _deflateDecoderSpec->Code(_bufInStream, _bufOutStream, nullptr, &blockSize64, nullptr);
...
HRESULT CHandler::Open2(IInStream *stream, IArchiveOpenCallback *openCallback)
{
  ...
  else
  {
    ...
    */
    ...
    if (refClusters != 0)
    {
      ...
      CByteBuffer refs;
      refs.Alloc(numBytes);
      RINOK(stream->Seek(refOffset, STREAM_SEEK_SET, nullptr));
  ...
  CByteBuffer table;
  {
    ...
    RINOK(stream->Seek(l1Offset, STREAM_SEEK_SET, nullptr));
  ...
  if (openCallback)
  {
    ...
    RINOK(openCallback->SetTotal(nullptr, &totalBytes));
  }
  ...
  for (UInt32 i = 0; i < l1Size; i++)
  {
    if (openCallback)
    {
      ...
      RINOK(openCallback->SetCompleted(nullptr, &numBytes));
    }
...

    {
      ...
      RINOK(stream->Seek(v, STREAM_SEEK_SET, nullptr));
...
REGISTER_ARC_I(
  "QCOW", "qcow qcow2 qcow2c", nullptr, 0xCA,
  k_Signature,
  0,
  0,
  nullptr)
