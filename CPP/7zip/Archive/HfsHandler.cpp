...
HRESULT CDatabase::ReadFile(const CFork &fork, CByteBuffer &buf, IInStream *inStream)
{
  ...
  FOR_VECTOR (i, fork.Extents)
  {
    ...
    if (e.Pos > Header.NumBlocks ||
      ...
      return S_FALSE;
    RINOK(inStream->Seek((UInt64)e.Pos << Header.BlockSizeLog, STREAM_SEEK_SET, nullptr));
...
HRESULT CDatabase::LoadAttrs(const CFork &fork, IInStream *inStream, IArchiveOpenCallback *progress)
{
  ...
  while (node != 0)
  {
    ...
    for (unsigned i = 0; i < desc.NumRecords; i++)
    {
      ...
      {
        UInt64 numFiles = 0;
        RINOK(progress->SetCompleted(&numFiles, nullptr));
      }
    }
    ...
    for (unsigned i = 0; i < desc.NumRecords; i++)
    {
      ...
      */
      ...
      if (progress && (Items.Size() & 0xFFF) == 0)
      {
        ...
        RINOK(progress->SetCompleted(&numItems, nullptr));
      }
    }
    node = desc.fLink;
  }
...
HRESULT CDatabase::Open2(IInStream *inStream, IArchiveOpenCallback *progress)
{
  ...
  if (progress)
  {
    ...
    RINOK(progress->SetTotal(&numFiles, nullptr));
  }
    ...
    IArchiveExtractCallback *extractCallback)
{
  ...
  for (i = 0; i < numBlocks; i++)
  {
    ...
    else
    {
      ...
      RINOK(_zlibDecoderSpec->Code(bufInStream, outStream, nullptr, &blockSize64, nullptr));
    ...
    Int32 testMode, IArchiveExtractCallback *extractCallback)
{
  ...
  NCompress::NZlib::CDecoder *_zlibDecoderSpec = nullptr;
    ...
    else if (item.UseAttr)
    {
      ...
      else
      {
        ...
        if (item.Method == kMethod_Attr)
        {
          ...
          HRESULT hres = _zlibDecoder->Code(bufInStream, realOutStream, nullptr, &item.UnpackSize, nullptr);
          if (hres != S_FALSE)
          {
            if (hres != S_OK)
              return hres;
    ...
    else
    {
      ...
      if (fork.IsOk(Header.BlockSizeLog))
      {
        ...
        for (extentIndex = 0; extentIndex < fork.Extents.Size(); extentIndex++)
        {
          ...
          RINOK(_stream->Seek((UInt64)e.Pos << Header.BlockSizeLog, STREAM_SEEK_SET, nullptr));
