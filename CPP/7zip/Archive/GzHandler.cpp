...
#include "../Compress/DeflateConst.h"
  ...
  namespace NGz {
    ...
    public:
      ...
      bool HeaderCrcIsPresent() const {
        return TestFlag(NFlags::kCrc);
      }
      bool ExtraFieldIsPresent() const {
        return TestFlag(NFlags::kExtra);
      }
      bool NameIsPresent() const {
        return TestFlag(NFlags::kName);
      }
      bool CommentIsPresent() const {
        return TestFlag(NFlags::kComment);
      }
      bool IsSupported() const {
        return (Flags & NFlags::kReserved) == 0;
      }
  ...
  STDMETHODIMP CHandler::Extract(const UInt32 *indices, UInt32 numItems,
    ...
    try {
      for(;;) {
        ...
        result = _decoderSpec->CodeResume(outStream, nullptr, progress);
