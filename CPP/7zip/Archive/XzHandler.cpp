...
namespace NArchive {
  namespace NXz {
    ...
    class CHandler :
      ...
      HRESULT Decode2(ISequentialInStream *seqInStream, ISequentialOutStream *outStream, CDecoder &decoder, ICompressProgressInfo *progress) {
    ...
    static SRes OpenCallbackProgress(void *pp, UInt64 inSize, UInt64 /* outSize */) {
      ...
      if(p->OpenCallback)
        p->Res = p->OpenCallback->SetCompleted(nullptr, &inSize);
      return (SRes)p->Res;
    }
    ...
    HRESULT CHandler::Open2(IInStream *inStream, /* UInt32 flags, */ IArchiveOpenCallback *callback) {
      ...
      if(callback) {
        RINOK(callback->SetTotal(nullptr, &_stat.PhySize));
      }
    ...
    STDMETHODIMP CSeekToSeqStream::Seek(Int64, UInt32, UInt64 *) {
      return E_NOTIMPL;
    }
