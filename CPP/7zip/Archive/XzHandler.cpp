...
class CHandler
  ...
  void Init() { CCommonMethodProps::InitCommon(); }
  ...
  HRESULT Decode(
      NCompress::NXz::CDecoder& decoder, ISequentialInStream* seqInStream,
 ...
 public:
  ...
  INTERFACE_IInArchive(;) STDMETHOD(OpenSeq)(ISequentialInStream* stream);
...
static const Byte kArcProps[] = {kpidMethod, kpidNumStreams, kpidNumBlocks, kpidClusterSize,
                                 kpidCharacts};
...
static const CMethodNamePair g_NamePairs[] = {
    {XZ_ID_Subblock, "SB"}, {XZ_ID_Delta, "Delta"}, {XZ_ID_X86, "BCJ"},
    {XZ_ID_PPC, "PPC"},     {XZ_ID_IA64, "IA64"},   {XZ_ID_ARM, "ARM"},
...
static void AddMethodString(AString& s, const CXzFilter& f) {
  ...
  if (f.propsSize > 0) {
    ...
    if (f.id == XZ_ID_LZMA2 && f.propsSize == 1)
      ...
      s += '[';
      for (UInt32 bi = 0; bi < f.propsSize; bi++) AddHexToString(s, f.props[bi]);
      s += ']';
    }
  }
}
...
static void AddCheckString(AString& s, const CXzs& xzs) {
  size_t i;
  UInt32 mask = 0;
  for (i = 0; i < xzs.num; i++) mask |= ((UInt32)1 << XzFlags_GetCheckType(xzs.streams[i].flags));
...
STDMETHODIMP CHandler::GetArchiveProperty(PROPID propID, PROPVARIANT* value) {
  ...
  switch (propID) {
    case kpidPhySize:
      if (_phySize_Defined) prop = _stat.InSize;
      break;
    case kpidNumStreams:
      if (_stat.NumStreams_Defined) prop = _stat.NumStreams;
      break;
    case kpidNumBlocks:
      if (_stat.NumBlocks_Defined) prop = _stat.NumBlocks;
      break;
    case kpidUnpackSize:
      if (_stat.UnpackSize_Defined) prop = _stat.OutSize;
      break;
    case kpidClusterSize:
      if (_stat.NumBlocks_Defined && _stat.NumBlocks > 1) prop = _maxBlocksSize;
      break;
    case kpidCharacts:
      if (_firstBlockWasRead) {
        AString s;
        if (XzBlock_HasPackSize(&_firstBlock)) s.Add_OptSpaced("BlockPackSize");
        if (XzBlock_HasUnpackSize(&_firstBlock)) s.Add_OptSpaced("BlockUnpackSize");
        if (!s.IsEmpty()) prop = s;
      }
      break;

    case kpidMethod:
      if (!_methodsString.IsEmpty()) prop = _methodsString;
      break;
    case kpidErrorFlags: {
      UInt32 v = 0;
      ...
      if (/* _stat.Unsupported */ sres == SZ_ERROR_UNSUPPORTED)
...
struct CLookToRead2_CPP : public CLookToRead2 {
  ...
  void Alloc(size_t allocSize) {
    ...
    if (buf) this->bufSize = allocSize;
  }
  ~CLookToRead2_CPP() { MyFree(buf); }
};
...
HRESULT CHandler::Open2(IInStream* inStream, /* UInt32 flags, */ IArchiveOpenCallback* callback) {
...

  {
    ...
    if (res != SZ_OK) return SRes_to_Open_HRESULT(res);
  ...
  if (!lookStream.buf) return E_OUTOFMEMORY;
  ...
  if (res == SZ_ERROR_PROGRESS) return (openWrap.Res == S_OK) ? E_FAIL : openWrap.Res;
  /*
      ...
      if (_blocks) {
        ...
        for (size_t si = xzs.p.num; si != 0;) {
          ...
          for (size_t bi = 0; bi < str.numBlocks; bi++) {
            ...
            if (bs.unpackSize != 0) {
              if (blockIndex >= _stat.NumBlocks) return E_FAIL;
        ...
        */
        if (_stat.OutSize != unpackPos) return E_FAIL;
        CBlockInfo& block = _blocks[blockIndex++];
        block.StreamFlags = 0;
        block.PackSize = 0;
        block.PackPos = 0;
...
static HRESULT DecodeBlock(
    CXzUnpackerCPP2& xzu, ISequentialInStream* seqInStream, unsigned streamFlags,
  ...
  if (!xzu.InBuf) {
    ...
    if (!xzu.InBuf) return E_OUTOFMEMORY;
  }
  ...
  for (;;) {
    if (inPos == inSize && readRes == S_OK) {
      ...
      if (rem > packRem) rem = (UInt32)packRem;
      if (rem != 0) readRes = seqInStream->Read(xzu.InBuf, rem, &inSize);
    }
    ...
    if (res != SZ_OK) {
      if (res == SZ_ERROR_CRC) return S_FALSE;
    ...
    if ((inLen == 0 && outLen == 0) || blockFinished) {
      if (packRem != 0 || !blockFinished || unpackSize != outPos) return S_FALSE;
      if (XzUnpacker_GetPackSizeForIndex(&xzu.p) != packSize) return S_FALSE;
      return S_OK;
    }
  }
}
...
STDMETHODIMP CInStream::Read(void* data, UInt32 size, UInt32* processedSize) {
  ...
  {
    if (_virtPos >= Size) return S_OK;  // (Size == _virtPos) ? S_OK: E_FAIL;
    {
  ...
  if (_virtPos < _cacheStartPos || _virtPos >= _cacheStartPos + _cacheSize) {
    ...
    if (_cache.Size() < unpackSize) return E_FAIL;
...
STDMETHODIMP CInStream::Seek(Int64 offset, UInt32 seekOrigin, UInt64* newPosition) {
  ...
  _virtPos = offset;
  if (newPosition) *newPosition = offset;
  return S_OK;
}
...
STDMETHODIMP CHandler::GetStream(UInt32 index, ISequentialInStream** stream) {
  ...
  *stream = NULL;
  ...
  if (!_stat.UnpackSize_Defined || _maxBlocksSize == 0  // 18.02
      || _maxBlocksSize > kMaxBlockSize_for_GetStream || _maxBlocksSize != (size_t)_maxBlocksSize)
    return S_FALSE;

  UInt64 memSize;
  if (!NSystem::GetRamSize(memSize)) memSize = (UInt64)(sizeof(size_t)) << 28;
  {
    if (_maxBlocksSize > memSize / 4) return S_FALSE;
  }
...
STDMETHODIMP CHandler::Extract(
  ...
  if (numItems != (UInt32)(Int32)-1 && (numItems != 1 || indices[0] != 0)) return E_INVALIDARG;

  if (_phySize_Defined) extractCallback->SetTotal(_stat.InSize);
  ...
  if (!decoder.MainDecodeSRes_wasUsed) return hres == S_OK ? E_FAIL : hres;

  Int32 opRes = Get_Extract_OperationResult(decoder);
  if (opRes == NExtract::NOperationResult::kOK && hres != S_OK)
    opRes = NExtract::NOperationResult::kDataError;
...
HRESULT CHandler::SetProperty(const wchar_t* nameSpec, const PROPVARIANT& value) {
  ...
  {
    HRESULT hres;
    if (SetCommonProperty(name, value, hres)) return hres;
  }
...
STDMETHODIMP CHandler::SetProperties(
  ...
  for (UInt32 i = 0; i < numProps; i++) { RINOK(SetProperty(names[i], values[i])); }
