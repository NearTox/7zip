    ...
    HRESULT CHandler::Open2(IInStream *stream, IArchiveOpenCallback *callback) {
      ...
      for(;;) {
        ...
        */
        if(callback) {
          if(_items.Size() == 1) {
            RINOK(callback->SetTotal(nullptr, &endPos));
          }
    ...
    HRESULT CHandler::SkipTo(UInt32 index) {
      while(_curIndex < index || !_latestIsRead) {
        if(_latestIsRead) {
          ...
          RINOK(copyCoderSpec->Code(_seqStream, nullptr, &packSize, &packSize, nullptr));
      ...
      for(i = 0; i < numItems || seqMode; i++) {
...

        {
          if(item->IsSymLink()) {
            ...
            if(!seqMode) {
              RINOK(_stream->Seek(item->GetDataPosition(), STREAM_SEEK_SET, nullptr));
            }
            streamSpec->Init(item->GetPackSizeAligned());
            RINOK(copyCoder->Code(inStream2, outStream, nullptr, nullptr, progress));
          }
      ...
      if(item.SparseBlocks.IsEmpty())
        ...
        if(_virtPos >= sb.Offset && relat < sb.Size) {
          ...
          if(_needStartSeek || _phyPos != phyPos) {
            RINOK(Handler->_stream->Seek(item.GetDataPosition() + phyPos, STREAM_SEEK_SET, nullptr));
