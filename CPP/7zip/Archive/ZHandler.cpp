...
namespace NArchive {
  ...
  STDMETHODIMP CHandler::Open(IInStream *stream,
    ...
    if (fullCheck)
    {
      ...
      outStreamSpec->SetStream(nullptr);
      outStreamSpec->Init();
      ...
      RINOK(stream->Seek(_streamStartPosition + kSignatureSize, STREAM_SEEK_SET, nullptr));
      HRESULT res = decoder->Code(stream, outStream, nullptr, nullptr, openCallback ? compressProgress : nullptr);
      if (res != S_OK)
        return S_FALSE;
  ...
  STDMETHODIMP CHandler::Extract(const UInt32 *indices, UInt32 numItems,
    ...
    int opRes;
    {
      HRESULT result = decoder->Code(_stream, outStream, nullptr, nullptr, progress);
      if(result == S_FALSE)
