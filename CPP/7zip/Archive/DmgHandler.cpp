...
static const CAppleName k_Names[] =
{
  ...
  { false, "ddm",  "DDM" },
  { false, nullptr,   "Apple_partition_map" },
  { false, nullptr,   " GPT " },
  { false, nullptr,   "MBR" },
  { false, nullptr,   "Driver" },
  { false, nullptr,   "Patches" }
};
...
HRESULT CHandler::ReadData(IInStream *stream, const CForkPair &pair, CByteBuffer &buf)
{
  ...
  buf.Alloc(size);
  RINOK(stream->Seek(_startPos + pair.Offset, STREAM_SEEK_SET, nullptr));
...
HRESULT CHandler::Open2(IInStream *stream)
{
  ...
  else
  {
    ...
    RINOK(stream->Seek(headerPos, STREAM_SEEK_SET, nullptr));
    RINOK(ReadStream_FALSE(stream, buf, HEADER_SIZE));
    if (!IsKoly(buf))
      return S_FALSE;
  }
  ...
  else
  {
    ...
    RINOK(stream->Seek(_startPos + xmlPair.Offset, STREAM_SEEK_SET, nullptr));
    ...
    #endif
    {
      ...
      UInt64 packPos = 0;
      {
        FOR_VECTOR (j, item.Blocks)
        {
          ...
          RINOK(_inStream->Seek(_startPos + item.StartPos + block.PackPos, STREAM_SEEK_SET, nullptr));
          ...
          switch (block.Type)
          {
            ...
            case METHOD_COPY:
              ...
              res = copyCoder->Code(inStream, outStream, nullptr, nullptr, progress);
              break;
            ...
            case METHOD_ZLIB:
            {
              res = zlibCoder->Code(inStream, outStream, nullptr, nullptr, progress);
              if (res == S_OK)
            ...
            case METHOD_BZIP2:
            {
              res = bzip2Coder->Code(inStream, outStream, nullptr, nullptr, progress);
              if (res == S_OK)
...
STDMETHODIMP CInStream::Read(void *data, UInt32 size, UInt32 *processedSize)
{
  ...
  if (_latestBlock < 0)
  {
    ...
    if (!block.IsZeroMethod() && block.Type != METHOD_COPY)
    {
      ...
      else
      {
        ...
        RINOK(Stream->Seek(_startPos + File->StartPos + block.PackPos, STREAM_SEEK_SET, nullptr));
        ...
        switch (block.Type)
        {
          ...
          case METHOD_ADC:
            ...
            res = adcCoder->Code(inStream, outStream, &block.PackSize, &block.UnpSize, nullptr);
            break;
          ...
          case METHOD_ZLIB:
            ...
            res = zlibCoder->Code(inStream, outStream, nullptr, nullptr, nullptr);
          ...
          case METHOD_BZIP2:
            ...
            res = bzip2Coder->Code(inStream, outStream, nullptr, nullptr, nullptr);
          ...
          case METHOD_LZFSE:
            ...
            res = lzfseCoder->Code(inStream, outStream, &block.PackSize, &block.UnpSize, nullptr);
            break;
  ...
  {
    RINOK(Stream->Seek(_startPos + File->StartPos + block.PackPos + offset, STREAM_SEEK_SET, nullptr));
