    ...
    Int32 testMode, IArchiveExtractCallback *extractCallback)
{
  ...
  if (_stream)
    RINOK(_stream->Seek(_item.HeaderSize, STREAM_SEEK_SET, nullptr));
  NCompress::NZlib::CDecoder *_decoderZlibSpec = nullptr;
  NCompress::NLzma::CDecoder *_decoderLzmaSpec = nullptr;
  ...
  HRESULT result = _decoder->Code(inStream2, outStream, nullptr, &unpackSize, progress);
    ...
    IArchiveUpdateCallback *updateCallback)
{
  ...
  */
  ...
  NCompress::NZlib::CEncoder *encoderZlibSpec = nullptr;
  NCompress::NLzma::CEncoder *encoderLzmaSpec = nullptr;
  ...
  else
  {
    ...
    RINOK(props.SetCoderProps(encoderZlibSpec->DeflateEncoderSpec, nullptr));
    item.MakeZlib();
  }
  RINOK(item.WriteHeader(outStream));
  ...
  RINOK(encoder->Code(fileInStream, outStream, nullptr, nullptr, progress));
  ...
  if (lzmaMode)
  {
    ...
    RINOK(outSeekStream->Seek(0, STREAM_SEEK_SET, nullptr));
    ...
    IArchiveUpdateCallback *updateCallback)
{
  ...
  return NCompress::CopyStream(_seqStream, outStream, nullptr);
}
...
STDMETHODIMP CHandler::OpenSeq(ISequentialInStream *stream)
{
  return OpenSeq2(stream, nullptr);
}
