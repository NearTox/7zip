...
STDMETHODIMP CFilterCoder::Code(
  ...
  while (!outSize || nowPos64 < *outSize) {
    ...
    if (pos > endPos) {
      // AES
      if (!inputFinished || pos > _bufSize) return E_FAIL;
      ...
      if (pos != Filter->Filter(_buf, pos)) return E_FAIL;
    }

    if (endPos == 0) return S_OK;

    UInt32 size = (pos != 0 ? pos : endPos);
    if (outSize) {
      UInt64 remSize = *outSize - nowPos64;
      if (size > remSize) size = (UInt32)remSize;
    }

    RINOK(WriteStream(outStream, _buf, size));
    nowPos64 += size;

    if (pos == 0) return S_OK;

    if (progress) RINOK(progress->SetRatioInfo(&nowPos64, &nowPos64));

    UInt32 i = 0;
    while (pos < endPos) _buf[i++] = _buf[pos++];
    pos = i;
  }
...
HRESULT CFilterCoder::Flush2() {
  ...
  if (_convPos != 0) {
    ...
    for (UInt32 i = 0; i < num; i++) _buf[i] = _buf[_convPos + i];
    _bufPos = num;
    _convPos = 0;
  }
...
STDMETHODIMP CFilterCoder::OutStreamFinish() {
  for (;;) {
    ...
    if (_convSize == 0)
      ...
      for (; _bufPos < _convSize; _bufPos++) _buf[_bufPos] = 0;
      _convSize = Filter->Filter(_buf, _bufPos);
...
STDMETHODIMP CFilterCoder::Read(void* data, UInt32 size, UInt32* processedSize) {
  ...
  while (size != 0) {
    ...
    if (_convSize > _bufPos) {
      // AES
      if (_convSize > _bufSize) return E_FAIL;
      if (!_encodeMode) return S_FALSE;
