...
#define REGISTER_CODEC_CREATE_2(name, cls, i) \
  static void* name() { return (void*)(i*)(new cls); }
...
#define REGISTER_CODECS(x)                                                                     \
  struct REGISTER_CODECS_NAME(x) {                                                             \
    REGISTER_CODECS_NAME(x)() {                                                                \
      for (unsigned i = 0; i < ARRAY_SIZE(g_CodecsInfo); i++) RegisterCodec(&g_CodecsInfo[i]); \
    }                                                                                          \
  };                                                                                           \
  static REGISTER_CODECS_NAME(x) g_RegisterCodecs;

#define REGISTER_CODEC_2(x, crDec, crEnc, id, name)     \
  REGISTER_CODEC_VAR{crDec, crEnc, id, name, 1, false}; \
  REGISTER_CODEC(x)
...
#define REGISTER_HASHER(cls, id, name, size)                                  \
  ...
  struct REGISTER_HASHER_NAME(cls) {                                          \
    REGISTER_HASHER_NAME(cls)() { RegisterHasher(&g_HasherInfo); }            \
  };                                                                          \
