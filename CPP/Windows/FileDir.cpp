...
#ifndef _UNICODE
  if (!g_IsNT) {
    if (::SetFileAttributes(fs2fas(path), attrib)) return true;
  } else
#endif
  {
    IF_USE_MAIN_PATH
    if (::SetFileAttributesW(fs2us(path), attrib)) return true;
#ifdef WIN_LONG_PATH
    if (USE_SUPER_PATH) {
      UString superPath;
      if (GetSuperPath(path, superPath, USE_MAIN_PATH))
...
#ifndef _UNICODE
  if (!g_IsNT) {
    if (::RemoveDirectory(fs2fas(path))) return true;
  } else
#endif
  {
    IF_USE_MAIN_PATH
    if (::RemoveDirectoryW(fs2us(path))) return true;
#ifdef WIN_LONG_PATH
    if (USE_SUPER_PATH) {
      UString superPath;
      if (GetSuperPath(path, superPath, USE_MAIN_PATH))
...
#ifndef _UNICODE
  if (!g_IsNT) {
    if (::MoveFile(fs2fas(oldFile), fs2fas(newFile))) return true;
  } else
#endif
  {
    IF_USE_MAIN_PATH_2(oldFile, newFile)
    if (::MoveFileW(fs2us(oldFile), fs2us(newFile))) return true;
#ifdef WIN_LONG_PATH
    if (USE_SUPER_PATH_2) {
      UString d1, d2;
...
#  endif
  {
    Func_CreateHardLinkW my_CreateHardLinkW = (Func_CreateHardLinkW)::GetProcAddress(
...
bool CreateComplexDir(CFSTR _path) {
...

  {
    ...
    if (attrib != INVALID_FILE_ATTRIBUTES && (attrib & FILE_ATTRIBUTE_DIRECTORY) != 0) return true;
  }

#  ifndef UNDER_CE

  if (IsDriveRootPath_SuperAllowed(_path)) return false;
...
#  endif
  ...
  for (;;) {
    if (CreateDir2(path)) break;
    if (::GetLastError() == ERROR_ALREADY_EXISTS) return false;
    pos = path.ReverseFind_PathSepar();
    if (pos < 0 || pos == 0) return false;

#if defined(_WIN32) && !defined(UNDER_CE)
    if (pos == 1 && IS_PATH_SEPAR(path[0])) return false;
    if (prefixSize >= (unsigned)pos + 1) return false;
#endif
...
bool DeleteFileAlways(CFSTR path) {
     ...
     SetFileAttrib("name:stream", ) changes attributes of main file. */
  {
    ...
    if (attrib != INVALID_FILE_ATTRIBUTES && (attrib & FILE_ATTRIBUTE_DIRECTORY) == 0 &&
      ...
      if (!SetFileAttrib(path, attrib & ~FILE_ATTRIBUTE_READONLY)) return false;
    }
  }

#ifndef _UNICODE
  if (!g_IsNT) {
    if (::DeleteFile(fs2fas(path))) return true;
  } else
#endif
  {
    ...
    IF_USE_MAIN_PATH
    if (::DeleteFileW(fs2us(path))) return true;
#ifdef WIN_LONG_PATH
    if (USE_SUPER_PATH) {
      UString superPath;
      if (GetSuperPath(path, superPath, USE_MAIN_PATH)) return BOOLToBool(::DeleteFileW(superPath));
    }
#endif
  }
  return false;
...
bool RemoveDirWithSubItems(const FString& path) {
  bool needRemoveSubItems = true;
  {
    ...
    if (fi.HasReparsePoint()) needRemoveSubItems = false;
  }
  ...
  if (needRemoveSubItems) {
    ...
    while (enumerator.Next(fi)) {
      ...
      s += fi.Name;
      if (fi.IsDir()) {
        if (!RemoveDirWithSubItems(s)) return false;
      } else if (!DeleteFileAlways(s))
        return false;
    }
  }

  if (!SetFileAttrib(path, 0)) return false;
...
static bool CreateTempFile(CFSTR prefix, bool addRandom, FString& path, NIO::COutFile* outFile) {
  ...
  for (unsigned i = 0; i < 100; i++) {
    ...
    if (outFile) {
      if (outFile->Create(path, false)) return true;
    } else {
...
bool CTempFile::Create(CFSTR prefix, NIO::COutFile* outFile) {
  ...
  if (!CreateTempFile(prefix, false, _path, outFile)) return false;
  _mustBeDeleted = true;
  return true;
}
...
bool CTempFile::CreateRandomInTempFolder(CFSTR namePrefix, NIO::COutFile* outFile) {
  ...
  if (!CreateTempFile(tempPath + namePrefix, true, _path, outFile)) return false;
  _mustBeDeleted = true;
  return true;
}

bool CTempFile::Remove() {
  if (!_mustBeDeleted) return true;
...
bool CTempFile::MoveTo(CFSTR name, bool deleteDestBefore) {
  ...
  if (deleteDestBefore) {
    if (NFind::DoesFileExist(name)) {
      ...
      if (!DeleteFileAlways(name)) return false;
    }
  }
  DisableDeleting();
