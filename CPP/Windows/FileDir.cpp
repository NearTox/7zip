      ...
      EXTERN_C_END
...
#endif
        {
          ...
          IF_USE_MAIN_PATH_2(newFileName, existFileName)
            if(my_CreateHardLinkW(fs2us(newFileName), fs2us(existFileName), nullptr))
              return true;
#ifdef WIN_LONG_PATH
          if(USE_SUPER_PATH_2) {
            UString d1, d2;
...
*/
...
#ifndef _UNICODE
        if(!g_IsNT) {
          if(::CreateDirectory(fs2fas(path), nullptr))
            return true;
        } else
#endif
        {
          IF_USE_MAIN_PATH
            if(::CreateDirectoryW(fs2us(path), nullptr))
              return true;
#ifdef WIN_LONG_PATH
      ...
      static bool CreateDir2(CFSTR path) {
...
#endif
        {
          ...
          if((!USE_MAIN_PATH || ::GetLastError() != ERROR_ALREADY_EXISTS) && USE_SUPER_PATH) {
            UString superPath;
            if(GetSuperPath(path, superPath, USE_MAIN_PATH)) {
              if(::CreateDirectoryW(superPath, nullptr))
                return true;
      ...
      bool CTempDir::Create(CFSTR prefix) {
        ...
        if(!MyGetTempPath(tempPath))
          return false;
        if(!CreateTempFile(tempPath + prefix, true, _path, nullptr))
          return false;
        _mustBeDeleted = true;
        return true;
      }
