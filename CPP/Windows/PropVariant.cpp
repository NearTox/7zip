...
HRESULT CPropVariant::Copy(const PROPVARIANT* pSrc) throw() {
  ...
  switch (pSrc->vt) {
    ...
    case VT_R8:
    case VT_CY:
    case VT_DATE: memmove((PROPVARIANT*)this, pSrc, sizeof(PROPVARIANT)); return S_OK;
  }
...
void CPropVariant::InternalCopy(const PROPVARIANT* pSrc) {
  ...
  if (FAILED(hr)) {
    if (hr == E_OUTOFMEMORY) throw kMemException;
    vt = VT_ERROR;
    scode = hr;
  }
}
...
int CPropVariant::Compare(const CPropVariant& a) throw() {
  if (vt != a.vt) return MyCompare(vt, a.vt);
  switch (vt) {
