...
#  ifndef _UNICODE
  if (!g_IsNT) {
    ...
    res = GetVolumeInformation(
        fs2fas(rootPath), v, MAX_PATH, volumeSerialNumber, maximumComponentLength, fileSystemFlags,
        f, MAX_PATH);
...
#  endif
  {
    ...
    res = GetVolumeInformationW(
        fs2us(rootPath), v, MAX_PATH, volumeSerialNumber, maximumComponentLength, fileSystemFlags,
        f, MAX_PATH);
    volumeName = v;
    fileSystemName = f;
  }
  ...
  if (!g_IsNT) {
    ...
    if (pGetDiskFreeSpaceEx) {
      ...
      sizeIsDetected = BOOLToBool(
          pGetDiskFreeSpaceEx(fs2fas(rootPath), &freeBytesToCaller2, &totalSize2, &freeSize2));
      totalSize = totalSize2.QuadPart;
      freeSize = freeSize2.QuadPart;
    }
    if (!::GetDiskFreeSpace(
            fs2fas(rootPath), &numSectorsPerCluster, &bytesPerSector, &numFreeClusters,
            &numClusters))
      return false;
  } else
#  endif
  {
    ...
    if (pGetDiskFreeSpaceEx) {
      ULARGE_INTEGER freeBytesToCaller2, totalSize2, freeSize2;
      sizeIsDetected = BOOLToBool(
          pGetDiskFreeSpaceEx(fs2us(rootPath), &freeBytesToCaller2, &totalSize2, &freeSize2));
      totalSize = totalSize2.QuadPart;
      freeSize = freeSize2.QuadPart;
    }
    if (!::GetDiskFreeSpaceW(
            fs2us(rootPath), &numSectorsPerCluster, &bytesPerSector, &numFreeClusters,
            &numClusters))
      return false;
  }
