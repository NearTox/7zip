...
typedef enum { My_FindStreamInfoStandard, My_FindStreamInfoMaxInfoLevel } MY_STREAM_INFO_LEVELS;
...
typedef WINBASEAPI HANDLE(WINAPI* FindFirstStreamW_Ptr)(
    LPCWSTR fileName, MY_STREAM_INFO_LEVELS infoLevel, LPVOID findStreamData, DWORD flags);
...
bool CFileInfo::IsDots() const throw() {
  if (!IsDir() || Name.IsEmpty()) return false;
...
bool CFindFileBase::Close() throw() {
  ...
  if (!::FindClose(_handle)) return false;
...
bool CFindFile::FindFirst(CFSTR path, CFileInfo& fi) {
...
#endif
  {
    ...
    if (_handle == INVALID_HANDLE_VALUE && USE_SUPER_PATH) {
      UString superPath;
      if (GetSuperPath(path, superPath, USE_MAIN_PATH)) _handle = ::FindFirstFileW(superPath, &fd);
    }
#endif
    if (_handle == INVALID_HANDLE_VALUE) return false;
    Convert_WIN32_FIND_DATA_to_FileInfo(fd, fi);
...
bool CFindFile::FindNext(CFileInfo& fi) {
  ...
  if (!g_IsNT) {
    WIN32_FIND_DATAA fd;
    if (!::FindNextFileA(_handle, &fd)) return false;
    Convert_WIN32_FIND_DATA_to_FileInfo(fd, fi);
  } else
#endif
  {
    WIN32_FIND_DATAW fd;
    if (!::FindNextFileW(_handle, &fd)) return false;
    Convert_WIN32_FIND_DATA_to_FileInfo(fd, fi);
  }
  return true;
}
...
struct CFindStreamLoader {
  CFindStreamLoader() {
    g_FindFirstStreamW = (FindFirstStreamW_Ptr)::GetProcAddress(
        ::GetModuleHandleA("kernel32.dll"), "FindFirstStreamW");
    g_FindNextStreamW = (FindNextStreamW_Ptr)::GetProcAddress(
        ::GetModuleHandleA("kernel32.dll"), "FindNextStreamW");
  }
} g_FindStreamLoader;
...
static void Convert_WIN32_FIND_STREAM_DATA_to_StreamInfo(
    const MY_WIN32_FIND_STREAM_DATA& sd, CStreamInfo& si) {
...
bool CFindStream::FindFirst(CFSTR path, CStreamInfo& si) {
  ...
  }
  {
    ...
    if (_handle == INVALID_HANDLE_VALUE) {
      if (::GetLastError() == ERROR_HANDLE_EOF) return false;
...
bool CFindStream::FindNext(CStreamInfo& si) {
  ...
  {
    MY_WIN32_FIND_STREAM_DATA sd;
    if (!g_FindNextStreamW(_handle, &sd)) return false;
    Convert_WIN32_FIND_STREAM_DATA_to_StreamInfo(sd, si);
  }
  return true;
}
...
WinXP-64 GetFileAttributes():
  ...
  \\Server\Share     - WORKS OK, but MSDN says:
                          GetFileAttributes for a network share, the function fails, and
GetLastError returns ERROR_BAD_NETPATH. You must specify a path to a subfolder on that share.
*/
...
DWORD GetFileAttrib(CFSTR path) {
...
#endif
  {
    IF_USE_MAIN_PATH {
      ...
      if (dw != INVALID_FILE_ATTRIBUTES) return dw;
    }
#ifdef WIN_LONG_PATH
    if (USE_SUPER_PATH) {
      UString superPath;
      if (GetSuperPath(path, superPath, USE_MAIN_PATH)) return ::GetFileAttributesW(superPath);
    }
#endif
...
bool CFileInfo::Find(CFSTR path) {
  ...
  if (IsDevicePath(path)) {
    ...
    if (inFile.SizeDefined) Size = inFile.Size;
    return true;
  }
#endif
    ...
    */
    ...
    if (isOk) {
      ...
      for (;;) {
        CStreamInfo si;
        bool found;
        if (!enumerator.Next(si, found)) return false;
        if (!found) {
        ...
        if (si.Name.IsEqualTo_NoCase(streamName)) {
          ...
          if (si.Name.Len() > kPostfixSize + 1) si.Name.DeleteFrom(si.Name.Len() - kPostfixSize);
          Name += us2fs(si.Name);
          Size = si.Size;
          IsAltStream = true;
          return true;
...
#if defined(_WIN32) && !defined(UNDER_CE)
  {
    ...
    */
    ...
    unsigned rootSize = 0;
    if (IsSuperPath(path)) rootSize = kSuperPathPrefixSize;
        ...
        if (prefixSize > 0 && path[prefixSize] != 0) {
          if (NName::FindSepar(path + prefixSize) < 0) {
            ...
            }
            {
              DWORD attrib = GetFileAttrib(path);
              if (isOK ||
                  attrib != INVALID_FILE_ATTRIBUTES && (attrib & FILE_ATTRIBUTE_DIRECTORY) != 0) {
                ClearBase();
...
bool CEnumerator::Next(CFileInfo& fi) {
  for (;;) {
    if (!NextAny(fi)) return false;
    if (!fi.IsDots()) return true;
  }
}
...
bool CFindChangeNotification::Close() throw() {
  if (!IsHandleAllocated()) return true;
  if (!::FindCloseChangeNotification(_handle)) return false;
  _handle = INVALID_HANDLE_VALUE;
  return true;
}
...
#  ifndef _UNICODE
  if (!g_IsNT) {
    ...
    if (newSize == 0 || newSize > size) return false;
    AString s;
    UINT32 prev = 0;
