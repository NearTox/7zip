...
bool FillLinkData(CByteBuffer& dest, const wchar_t* path, bool isSymLink) {
  ...
  if (!isAbs && !isSymLink) return false;
  ...
  if (add_Prefix_Len != 0) WriteString(p + subOffs, k_LinkPrefix);
...
bool CReparseAttr::Parse(const Byte* p, size_t size, DWORD& errorCode) {
  ...
  */
  ...
  if ((subOffs & 1) != 0 || subOffs > len || len - subOffs < subLen) return false;
  if ((printOffs & 1) != 0 || printOffs > len || len - printOffs < printLen) return false;
...
bool CReparseAttr::IsOkNamePair() const {
  if (IsLinkPrefix(SubsName)) {
    if (!IsDrivePath(SubsName.Ptr(k_LinkPrefix_Size))) return PrintName.IsEmpty();
    if (wcscmp(SubsName.Ptr(k_LinkPrefix_Size), PrintName) == 0) return true;
  }
...
UString CReparseAttr::GetPath() const {
  ...
  if (IsLinkPrefix(s)) {
    ...
    if (IsDrivePath(s.Ptr(k_LinkPrefix_Size))) s.DeleteFrontal(k_LinkPrefix_Size);
  }
  return s;
}
...
bool GetReparseData(CFSTR path, CByteBuffer& reparseData, BY_HANDLE_FILE_INFORMATION* fileInfo) {
  reparseData.Free();
  CInFile file;
  if (!file.OpenReparse(path)) return false;

  if (fileInfo) file.GetFileInformation(fileInfo);
...
#  ifdef _WIN32
  if (pos == 2 && path2[1] == L':') return true;  // we don't create Disk folder;
#  endif
...
bool SetReparseData(CFSTR path, bool isDir, const void* data, DWORD size) {
  ...
  if (fi.Find(path)) {
    ...
    if (isDir) {
      if (!NDir::CreateComplexDir(path)) return false;
    } else {
      CreatePrefixDirOfFile(path);
      COutFile file;
      if (!file.Create(path, CREATE_NEW)) return false;
    }
  }

  COutFile file;
  if (!file.Open(
          path, FILE_SHARE_WRITE, OPEN_EXISTING,
