...
void NormalizeDirPathPrefix(
    FString& dirPath);  // ensures that it ended with '\\', if dirPath is not epmty
...
bool IsNetworkShareRootPath(
    CFSTR s) throw(); /* \\?\UNC\SERVER\share or \\SERVER\share or with slash */

bool IsDrivePath_SuperAllowed(
    CFSTR s) throw();  // first chars are drive chars like "a:\" or "\\?\a:\"
...
#  define USE_MAIN_PATH_2                                  \
    (__useSuperPathType1 != kSuperPathType_UseOnlySuper && \
     __useSuperPathType2 != kSuperPathType_UseOnlySuper)

#  define USE_SUPER_PATH (__useSuperPathType != kSuperPathType_UseOnlyMain)
#  define USE_SUPER_PATH_2                                \
    (__useSuperPathType1 != kSuperPathType_UseOnlyMain || \
     __useSuperPathType2 != kSuperPathType_UseOnlyMain)

#  define IF_USE_MAIN_PATH                              \
    int __useSuperPathType = GetUseSuperPathType(path); \
    if (USE_MAIN_PATH)
