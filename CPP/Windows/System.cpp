...
UInt32 CountAffinity(DWORD_PTR mask) {
  UInt32 num = 0;
  for (unsigned i = 0; i < sizeof(mask) * 8; i++) num += (UInt32)((mask >> i) & 1);
  return num;
}
...
#  ifdef _WIN64
  ...
  if (!::GlobalMemoryStatusEx(&stat)) return false;
...
#    ifndef UNDER_CE
  GlobalMemoryStatusExP globalMemoryStatusEx = (GlobalMemoryStatusExP)::GetProcAddress(
      ::GetModuleHandle(TEXT("kernel32.dll")), "GlobalMemoryStatusEx");
