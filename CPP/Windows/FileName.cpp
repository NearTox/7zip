...
void NormalizeDirPathPrefix(FString& dirPath) {
  ...
  if (!IsPathSepar(dirPath.Back())) dirPath.Add_PathSepar();
}
#endif
...
bool IsAltPathPrefix(CFSTR s) throw() {
  ...
  if (IsDevicePath(s)) return false;
  if (IsSuperPath(s)) {
    s += kSuperPathPrefixSize;
    len -= kSuperPathPrefixSize;
  }
  if (len == 2 && IsDrivePath2(s)) return false;
#endif
...
#  define IS_DEVICE_PATH(s) \
    (IS_SEPAR((s)[0]) && IS_SEPAR((s)[1]) && (s)[2] == '.' && IS_SEPAR((s)[3]))
#  define IS_SUPER_PREFIX(s) \
    (IS_SEPAR((s)[0]) && IS_SEPAR((s)[1]) && (s)[2] == '?' && IS_SEPAR((s)[3]))
#  define IS_SUPER_OR_DEVICE_PATH(s) \
    (IS_SEPAR((s)[0]) && IS_SEPAR((s)[1]) && ((s)[2] == '?' || (s)[2] == '.') && IS_SEPAR((s)[3]))

#  define IS_UNC_WITH_SLASH(s)                                               \
    (((s)[0] == 'U' || (s)[0] == 'u') && ((s)[1] == 'N' || (s)[1] == 'n') && \
     ((s)[2] == 'C' || (s)[2] == 'c') && IS_SEPAR((s)[3]))
...
#  else
  ...
  if (!IS_DEVICE_PATH(s)) return false;
  unsigned len = MyStringLen(s);
  if (len == 6 && s[5] == ':') return true;
  if (len < 18 || len > 22 ||
      !IsString1PrefixedByString2(s + kDevicePathPrefixSize, "PhysicalDrive"))
    return false;
...
bool IsNetworkPath(CFSTR s) throw() {
  if (!IS_SEPAR(s[0]) || !IS_SEPAR(s[1])) return false;
  if (IsSuperUncPath(s)) return true;
  FChar c = s[2];
  return (c != '.' && c != '?');
}

unsigned GetNetworkServerPrefixSize(CFSTR s) throw() {
  if (!IS_SEPAR(s[0]) || !IS_SEPAR(s[1])) return 0;
...
// bool IsDriveName2(const wchar_t *s) throw() { return IS_LETTER_CHAR(s[0]) && s[1] == ':' && s[2]
// == 0; }
...
bool IsDrivePath_SuperAllowed(CFSTR s) throw() {
  if (IsSuperPath(s)) s += kSuperPathPrefixSize;
...
bool IsAbsolutePath(const wchar_t* s) throw() { return IS_SEPAR(s[0]) || IsDrivePath2(s); }

int FindAltStreamColon(CFSTR path) throw() {
  unsigned i = 0;
  if (IsDrivePath2(path)) i = 2;
  int colonPos = -1;
  for (;; i++) {
    FChar c = path[i];
...
static unsigned GetRootPrefixSize_Of_SimplePath(CFSTR s) {
  if (IsDrivePath(s)) return kDrivePrefixSize;
  if (!IS_SEPAR(s[0])) return 0;
  if (s[1] == 0 || !IS_SEPAR(s[1])) return 1;
...
unsigned GetRootPrefixSize(CFSTR s) throw() {
  if (IS_DEVICE_PATH(s)) return kDevicePathPrefixSize;
  if (IsSuperPath(s)) return GetRootPrefixSize_Of_SuperPath(s);
    ...
    if (c == '.' && (i == 0 || IS_SEPAR(s[i - 1]))) {
      ...
      if (c1 == '.') {
        ...
        if (IS_SEPAR(c2) || c2 == 0) {
          ...
          for (;; k--) {
            if (k < 0) return false;
            if (!IS_SEPAR(s[(unsigned)k])) break;
          }
...
#  ifdef LONG_PATH_DOTS_FOLDERS_PARSING
    if ((s)[2] != '.')
      if (AreThereDotsFolders(s + kSuperPathPrefixSize)) return kSuperPathType_UseOnlySuper;
#  endif
  ...
  for (unsigned i = 0;; i++) {
    FChar c = s[i];
    if (c == 0) return kSuperPathType_UseMainAndSuper;
    if (c == '.' || c == ' ') {
      FChar c2 = s[i + 1];
      if (c2 == 0 || IS_SEPAR(c2)) {
        // if it's "." or "..", it's not bad name.
        if (c == '.') {
          if (i == 0 || IS_SEPAR(s[i - 1])) continue;
          if (s[i - 1] == '.') {
...
static bool GetSuperPathBase(CFSTR s, UString& res) {
  ...
  FChar c = s[0];
  if (c == 0) return true;
  if (c == '.' && (s[1] == 0 || (s[1] == '.' && s[2] == 0))) return true;
...
#  ifdef LONG_PATH_DOTS_FOLDERS_PARSING
    ...
    if (!AreThereDotsFolders(s + kSuperPathPrefixSize)) return true;

    UString temp = fs2us(s);
    unsigned fixedSize = GetRootPrefixSize_Of_SuperPath(temp);
    if (fixedSize == 0) return true;

    UString rem = &temp[fixedSize];
    if (!ResolveDotsFolders(rem)) return true;
...
#  endif
  ...
  if (IS_SEPAR(c)) {
    if (IS_SEPAR(s[1])) {
      ...
      */
      UString rem = &temp[fixedSize];
      if (!ResolveDotsFolders(rem)) return false;
      res += kSuperUncPrefix;
      temp.DeleteFrom(fixedSize);
      res += temp;
      res += rem;
    ...
    if (IsDrivePath2(s)) {
      UString temp = fs2us(s);
      unsigned prefixSize = 2;
      if (IsDrivePath(s)) prefixSize = kDrivePrefixSize;
  ...
  UString curDir;
  if (!GetCurDir(curDir)) return false;
  ...
  if (IsSuperPath(curDir)) {
    ...
    if (IsDrivePath(curDir)) {
      ...
      if (!IsPathSepar(curDir[0]) || !IsPathSepar(curDir[1])) return false;
      fixedSizeStart = 2;
  ...
  if (!ResolveDotsFolders(temp)) return false;
  if (superMarker) res += superMarker;
...
/*
  ...
  GetSuperPathBase  superPath.IsEmpty() onlyIfNew
     ...
     true            true             false       use main path as Super Path, we don't try mainMath
                                                  That case is possible now if GetCurDir returns
  unknow type of path (not drive and not network)
...
bool GetSuperPaths(CFSTR s1, CFSTR s2, UString& d1, UString& d2, bool onlyIfNew) {
  if (!GetSuperPathBase(s1, d1) || !GetSuperPathBase(s2, d2)) return false;
  if (d1.IsEmpty() && d2.IsEmpty() && onlyIfNew) return false;
...
bool GetFullPath(CFSTR dirPrefix, CFSTR s, FString& res) {
  ...
  if (prefixSize != 0) {
    if (!AreThereDotsFolders(s + prefixSize)) return true;

    UString rem = fs2us(s + prefixSize);
    if (!ResolveDotsFolders(rem)) return true;  // maybe false;
