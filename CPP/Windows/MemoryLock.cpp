...
extern "C" {
typedef BOOL(WINAPI* Func_OpenProcessToken)(
...
typedef BOOL(WINAPI* Func_AdjustTokenPrivileges)(
    HANDLE TokenHandle, BOOL DisableAllPrivileges, PTOKEN_PRIVILEGES NewState, DWORD BufferLength,
...
unsigned Get_LargePages_RiskLevel() {
  ...
  if (!func) return 0;
  func(&vi);
  return (vi.dwPlatformId == VER_PLATFORM_WIN32_NT && vi.dwMajorVersion + vi.dwMinorVersion == 10 &&
          vi.dwBuildNumber <= 16299)
             ? 1
             : 0;
}
