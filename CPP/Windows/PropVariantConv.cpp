...
bool ConvertUtcFileTimeToString(const FILETIME& utc, char* s, int level) throw() {
  *s = 0;
  FILETIME ft;
  if (!FileTimeToLocalFileTime(&utc, &ft)) return false;

  SYSTEMTIME st;
  if (!BOOLToBool(FileTimeToSystemTime(&ft, &st))) return false;
      ...
      if (level > kTimestampPrintLevel_SEC) {
        ...
        */

        {
          ...
          if (numDigits > (unsigned)level) numDigits = (unsigned)level;
          s += numDigits;
        }
      }
    }
