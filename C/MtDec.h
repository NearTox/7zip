...
typedef enum {
  ...
  MTDEC_PARSE_END        // end of block threading. But we still can return to threading after
                         // Write(&needContinue)
} EMtDecParseState;
...
typedef struct {
  ...
  SRes (*Code)(
      void* p, unsigned coderIndex, const Byte* src, size_t srcSize, int srcFinished,
  ...
  /* Write() must be called, if Parse() was called
    ...
    out:
      ...
      if (*canRecode), we didn't flush current block data, so we still can decode current block
    later.
  */
  SRes (*Write)(
      void* p, unsigned coderIndex, BoolInt needWriteToStream, const Byte* src, size_t srcSize,
      // int srcFinished,
