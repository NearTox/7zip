...
static WRes ArEvent_OptCreate_And_Reset(CEvent* p) {
  if (Event_IsCreated(p)) return Event_Reset(p);
...
static SRes MtDecThread_CreateAndStart(CMtDecThread* t) {
  ...
  // wres = 17; // for test
  if (wres == 0) {
    if (Thread_WasCreated(&t->thread)) return SZ_OK;
...
static void MtDecThread_CloseThread(CMtDecThread* t) {
  if (Thread_WasCreated(&t->thread)) {
    Event_Set(&t->canWrite); /* we can disable it. There are no threads waiting canWrite in normal
                                cases */
    Event_Set(&t->canRead);
...
static void MtDec_CloseThreads(CMtDec* p) {
  unsigned i;
  for (i = 0; i < MTDEC__THREADS_MAX; i++) MtDecThread_CloseThread(&p->threads[i]);
}
...
static SRes MtDec_Progress_GetError_Spec(
    CMtDec* p, UInt64 inSize, UInt64 outSize, UInt64 interruptIndex, BoolInt* wasInterrupted) {
  SRes res;
  CriticalSection_Enter(&p->mtProgress.cs);
  ...
  if (p->mtProgress.res == SZ_OK && p->mtProgress.progress)
    if (ICompressProgress_Progress(
            p->mtProgress.progress, p->mtProgress.totalInSize, p->mtProgress.totalOutSize) != SZ_OK)
...
static WRes ThreadFunc2(CMtDecThread* t) {
  ...
  for (;;) {
    ...
    if (res == SZ_OK && !wasInterrupted) {
        ...
        for (;;) {
...

          {
            ...
            if (crossSize != 0) {
              ...
              PRF(printf(
                  "\ncross : crossStart = %7d  crossEnd = %7d finish = %1d", (int)p->crossStart,
                  (int)p->crossEnd, (int)finish));
            } else {
              size = p->inBufSize;
              ...
              inDataSize += size;
              // inDataSize_Full = inDataSize;
              if (!prev) inDataSize_Start = size;

              p->readProcessed += size;
              finish = (size != p->inBufSize);
              if (finish) p->readWasFinished = True;
              ...
              if (inDataSize - inPrev >= MTDEC_ProgessStep) {
                res = MtDec_Progress_GetError_Spec(p, 0, 0, blockIndex, &wasInterrupted);
                if (res != SZ_OK || wasInterrupted) break;
                inPrev = inDataSize;
              }
            }

            {
              ...
              // printf("\n\n%12I64u %12I64u", (UInt64)p->mtProgress.totalInSize,
              // (UInt64)p->mtProgress.totalOutSize);
              ...
              if (
                ...
                if (crossSize != 0) memcpy(data, parseData, size);
                p->crossStart = 0;
                p->crossEnd = 0;
                break;
              }
              ...
              if (parse.state != MTDEC_PARSE_CONTINUE || finish) {
                ...
                if (parse.state == MTDEC_PARSE_END) finish = True;
                ...
                if (parse.state == MTDEC_PARSE_END) {
                  ...
                  if (crossSize != 0)
                    memcpy(
                        data + parse.srcSize, parseData + parse.srcSize,
                  ...
                  if (!prev) inDataSize_Start = parse.srcSize;
                  break;
                }

                {
                  ...
                  if (crossSize != 0)
                    ...
                    // partial parsed - is not in initial cross block - we need to copy new data to
                    // cross block
                  ...
                  // inDataSize_Full = inDataSize;
                  if (!prev) inDataSize_Start = parse.srcSize;  // it's partial size (parsed only)
      ...
      if (res == SZ_OK) res = MtDec_GetError_Spec(p, blockIndex, &wasInterrupted);
    }
    ...
    if (res != SZ_OK || wasInterrupted) finish = True;
    ...
    // each call of Event_Set(&nextThread->canRead) must be followed by call of
    // Event_Set(&nextThread->canWrite) if ( !finish ) we must call Event_Set(&nextThread->canWrite)
    // in any case if (  finish ) we switch to single-thread mode and there are 2 ways at the end of
    // current iteration (current block):
    ...
    if (res == SZ_OK && needCode && codeRes == SZ_OK) {
      ...
      for (;;) {
        ...
        if (isStartBlock)
          ...
          inSize = p->inBufSize;
          if (inSize > rem) inSize = (size_t)rem;
        }

        inCodePos += inSize;
        stop = True;

        codeRes = p->mtCallback->Code(
            p->mtCallbackObject, t->index, (const Byte*)MTDEC__DATA_PTR_FROM_LINK(link), inSize,
        ...
        if (stop || inCodePos == inDataSize) break;
...

    {
      ...
      if (p->wasInterrupted)
        ...
        if (codeRes != SZ_OK)  // || !needCode // check it !!!
        {
          ...
          p->codeRes = codeRes;
          if (codeRes == SZ_ERROR_MEM) isAllocError = True;
        }
      ...
      if (needWrite) {
        ...
        res = p->mtCallback->Write(
            p->mtCallbackObject, t->index,
            res == SZ_OK && needWriteToStream && !wasInterrupted,  // needWrite
            afterEndData, afterEndData_Size, &needContinue, &canRecode);
        ...
        if (res != SZ_OK || (!needContinue && !finish)) {
      ...
      if (canRecode)
        if (!needCode || res != SZ_OK || p->wasInterrupted || codeRes != SZ_OK || wasInterrupted ||
            p->numFilledThreads != 0 || isErrorMode) {
          if (p->numFilledThreads == 0) p->filledThreadStart = t->index;
...
static THREAD_FUNC_RET_TYPE THREAD_FUNC_CALL_TYPE ThreadFunc1(void* pp) {
  ...
  res = ThreadFunc2(t);
  p = t->mtDec;
  if (res == 0) return p->exitThreadWRes;
  {
    // it's unexpected situation for some threading function error
    if (p->exitThreadWRes == 0) p->exitThreadWRes = res;
    PRF(printf("\nthread exit error = %d\n", res));
    p->exitThread = True;
...

  {
    ...
    for (i = 0; i < MTDEC__THREADS_MAX; i++)
      if (i > p->numStartedThreads ||
          p->numFilledThreads <= (i >= p->filledThreadStart
...
const Byte* MtDec_Read(CMtDec* p, size_t* inLim) {
  while (p->numFilledThreads != 0) {
    ...
    if (*inLim != 0) {
      ...
      if (t->inDataSize == 0) {
        MtDecThread_FreeInBufs(t);
        if (--p->numFilledThreads == 0) break;
        if (++p->filledThreadStart == p->numStartedThreads) p->filledThreadStart = 0;
...
static void MtDec_Free(CMtDec* p) {
  ...
  for (i = 0; i < MTDEC__THREADS_MAX; i++) MtDecThread_Destruct(&p->threads[i]);
...
SRes MtDec_Code(CMtDec* p) {
...

  {
    ...
    if (numThreads > MTDEC__THREADS_MAX) numThreads = MTDEC__THREADS_MAX;
  ...
  if (p->inBufSize != p->allocatedBufsSize) {
    for (i = 0; i < MTDEC__THREADS_MAX; i++) {
      CMtDecThread* t = &p->threads[i];
      if (t->inBuf) MtDecThread_FreeInBufs(t);
    }
    if (p->crossBlock) {
      ISzAlloc_Free(p->alloc, p->crossBlock);
      p->crossBlock = NULL;
...

  {
    ...
    if (sres != 0) p->threadingErrorSRes = sres;

    if (
        // wres == 0
        // wres != 0
        // || p->mtc.codeRes == SZ_ERROR_MEM
        p->isAllocError || p->threadingErrorSRes != SZ_OK || p->overflow) {
      // p->needContinue = True;
    } else
      p->needContinue = False;

    if (p->needContinue) return SZ_OK;
