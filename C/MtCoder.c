...
static SRes MtCoderThread_CreateAndStart(CMtCoderThread* t) {
  ...
  if (wres == 0) {
    t->stop = False;
    if (!Thread_WasCreated(&t->thread)) wres = Thread_Create(&t->thread, ThreadFunc, t);
    if (wres == 0) wres = Event_Set(&t->startEvent);
  }
...
static SRes ThreadFunc2(CMtCoderThread* t) {
  ...
  for (;;) {
    ...
    /* after Event_Wait(&mtc->readEvent) we must call Event_Set(&mtc->readEvent) in any case to
    ...
    if (++mtc->blockIndex >= mtc->numBlocksMax) mtc->blockIndex = 0;

    bufIndex = (unsigned)(int)-1;

    if (res == SZ_OK) res = MtProgress_GetError(&mtc->mtProgress);

    if (res != SZ_OK) finished = True;
    ...
    if (finished) mtc->stopReading = True;

    RINOK_THREAD(Event_Set(&mtc->readEvent))

    if (res2 != SZ_OK) return res2;
    ...
    if (res == SZ_OK) {
      ...
      res = mtc->mtCallback->Code(
          mtc->mtCallbackObject, t->index, bufIndex, mtc->inStream ? t->inBuf : inData, size,
          finished);

      // MtProgress_Reinit(&mtc->mtProgress, t->index);

      if (res != SZ_OK) MtProgress_SetError(&mtc->mtProgress, res);
    }
...
#  else
    {
      ...
      if (wi != bi) {
        if (res != SZ_OK || finished) return 0;
        continue;
      }

      if (mtc->writeRes != SZ_OK) res = mtc->writeRes;
      ...
      for (;;) {
...

        {
          ...
          if (res == SZ_OK && block->res != SZ_OK) res = block->res;
...
#  endif
    ...
    if (finished || res != SZ_OK) return 0;
  }
}
...
static THREAD_FUNC_RET_TYPE THREAD_FUNC_CALL_TYPE ThreadFunc(void* pp) {
  ...
  for (;;) {
    if (Event_Wait(&t->startEvent) != 0) return SZ_ERROR_THREAD;
    if (t->stop) return 0;
    {
...
#  ifndef MTCODER__USE_WRITE_THREAD
      {
        ...
        if (numFinished == mtc->numStartedThreads)
          if (Event_Set(&mtc->finishedEvent) != 0) return SZ_ERROR_THREAD;
      }
#  endif
    }
  }
...
#  ifdef MTCODER__USE_WRITE_THREAD
  for (i = 0; i < MTCODER__BLOCKS_MAX; i++) Event_Construct(&p->writeEvents[i]);
#  else
...
static void MtCoder_Free(CMtCoder* p) {
  ...
  */
  ...
  for (i = 0; i < MTCODER__THREADS_MAX; i++) MtCoderThread_Destruct(&p->threads[i]);

  Event_Close(&p->readEvent);
  Semaphore_Close(&p->blocksSemaphore);

#  ifdef MTCODER__USE_WRITE_THREAD
  for (i = 0; i < MTCODER__BLOCKS_MAX; i++) Event_Close(&p->writeEvents[i]);
#  else
...
SRes MtCoder_Code(CMtCoder* p) {
  ...
  if (numThreads > MTCODER__THREADS_MAX) numThreads = MTCODER__THREADS_MAX;
  ...
  if (numBlocksMax > MTCODER__BLOCKS_MAX) numBlocksMax = MTCODER__BLOCKS_MAX;
...
#  endif
  ...
  for (i = 0; i < MTCODER__BLOCKS_MAX - 1; i++) p->freeBlockList[i] = i + 1;
...
#  ifndef MTCODER__USE_WRITE_THREAD
  p->writeIndex = 0;
  p->writeRes = SZ_OK;
  for (i = 0; i < MTCODER__BLOCKS_MAX; i++) p->ReadyBlocks[i] = False;
...

      {
        ...
        if (bufIndex != (unsigned)(int)-1) {
          if (res == SZ_OK) {
            ...
            if (res != SZ_OK) MtProgress_SetError(&p->mtProgress, res);
          }
...
#  endif
  ...
  if (res == SZ_OK) res = p->readRes;

  if (res == SZ_OK) res = p->mtProgress.res;

#  ifndef MTCODER__USE_WRITE_THREAD
  if (res == SZ_OK) res = p->writeRes;
#  endif

  if (res != SZ_OK) MtCoder_Free(p);
  return res;
}
