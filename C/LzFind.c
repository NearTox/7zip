...
static void MatchFinder_ReadBlock(CMatchFinder* p) {
  ...
  if (p->directInput) {
    ...
    if (curSize > p->directInputRem) curSize = (UInt32)p->directInputRem;
...
void MatchFinder_MoveBlock(CMatchFinder* p) {
  memmove(
      p->bufferBase, p->buffer - p->keepSizeBefore,
...
void MatchFinder_Construct(CMatchFinder* p) {
  ...
  for (i = 0; i < 256; i++) {
    UInt32 r = (UInt32)i;
    unsigned j;
    for (j = 0; j < 8; j++) r = (r >> 1) ^ (kCrcPoly & ((UInt32)0 - (r & 1)));
    p->crc[i] = r;
  }
}
  ...
  if (LzInWindow_Create(p, sizeReserv, alloc)) {
    ...
    p->matchMaxLen = matchMaxLen;
    {
      ...
      if (p->numHashBytes == 2)
        ...
        hs = historySize;
        if (hs > p->expectedDataSize) hs = (UInt32)p->expectedDataSize;
        if (hs != 0) hs--;
        hs |= (hs >> 1);
        hs |= (hs >> 2);
        hs |= (hs >> 4);
...
void MatchFinder_Init_LowHash(CMatchFinder* p) {
  ...
  for (i = 0; i < numItems; i++) items[i] = kEmptyHashValue;
}
...
void MatchFinder_Init_3(CMatchFinder* p, int readData) {
  ...
  if (readData) MatchFinder_ReadBlock(p);
...
static UInt32* Hc_GetMatchesSpec(
  ...
  for (;;)
  {
      ...
      return distances;
    {
      ...
      curMatch = son[_cyclicBufferPos - delta + ((delta > _cyclicBufferPos) ? _cyclicBufferSize :
  0)]; if (pb[maxLen] == cur[maxLen] && *pb == *cur)
      {
        UInt32 len = 0;
        while (++len != lenLimit)
          if (pb[len] != cur[len])
...
UInt32* GetMatchesSpec1(
  ...
  for (;;) {
    ...
    }
    {
      CLzRef* pair =
          son +
          ((size_t)(_cyclicBufferPos - delta + ((delta > _cyclicBufferPos) ? _cyclicBufferSize : 0))
           << 1);
      const Byte* pb = cur - delta;
...
#define GET_MATCHES_FOOTER(offset, maxLen)                                                                                   \
  offset =                                                                                                                   \
      (unsigned)(GetMatchesSpec1((UInt32)lenLimit, curMatch, MF_PARAMS(p), distances + offset, (UInt32)maxLen) - distances); \
  MOVE_POS_RET;
...
static UInt32 Hc4_MatchFinder_GetMatches(CMatchFinder* p, UInt32* distances) {
  ...
  offset =
      (unsigned)(Hc_GetMatchesSpec(lenLimit, curMatch, MF_PARAMS(p), distances + offset, maxLen) - (distances));
  MOVE_POS_RET
}
...
UInt32 Hc3Zip_MatchFinder_GetMatches(CMatchFinder* p, UInt32* distances) {
  ...
  p->hash[hv] = p->pos;
  offset =
      (unsigned)(Hc_GetMatchesSpec(lenLimit, curMatch, MF_PARAMS(p), distances, 2) - (distances));
  MOVE_POS_RET
}
