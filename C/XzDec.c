  ...
  *value = 0;
  ...
  for (i = 0; i < limit;) {
    ...
    *value |= (UInt64)(b & 0x7F) << (7 * i++);
    if ((b & 0x80) == 0) return (b == 0 && i != 1) ? 0 : i;
  }
  return 0;
}
...
static void BraState_Free(void* pp, ISzAllocPtr alloc) { ISzAlloc_Free(alloc, pp); }
...
static SRes BraState_SetProps(void* pp, const Byte* props, size_t propSize, ISzAllocPtr alloc) {
  ...
  if (p->methodId == XZ_ID_Delta) {
    if (propSize != 1) return SZ_ERROR_UNSUPPORTED;
    ...
    if (propSize == 4) {
      ...
      switch (p->methodId) {
        ...
        case XZ_ID_ARM:
        case XZ_ID_SPARC:
          if ((v & 3) != 0) return SZ_ERROR_UNSUPPORTED;
          break;
        case XZ_ID_ARMT:
          if ((v & 1) != 0) return SZ_ERROR_UNSUPPORTED;
          break;
        case XZ_ID_IA64:
          if ((v & 0xF) != 0) return SZ_ERROR_UNSUPPORTED;
          break;
      }
      p->ip = v;
    } else if (propSize != 0)
...
static void BraState_Init(void* pp) {
  ...
  if (p->methodId == XZ_ID_Delta) Delta_Init(p->deltaState);
}
...
static SRes BraState_Code2(
    void* pp, Byte* dest, SizeT* destLen, const Byte* src, SizeT* srcLen, int srcWasFinished,
...
SRes BraState_SetFromMethod(IStateCoder* p, UInt64 id, int encodeMode, ISzAllocPtr alloc) {
  CBraState* decoder;
  if (id < XZ_ID_Delta || id > XZ_ID_SPARC) return SZ_ERROR_UNSUPPORTED;
  decoder = p->p;
  if (!decoder) {
    decoder = (CBraState*)ISzAlloc_Alloc(alloc, sizeof(CBraState));
    if (!decoder) return SZ_ERROR_MEM;
    p->p = decoder;
...
static SRes SbState_Code2(
  ...
  *status =
      (*destLen == 0 && *srcLen == 0) ? CODER_STATUS_FINISHED_WITH_MARK : CODER_STATUS_NOT_FINISHED;
  return res;
}
...
static void Lzma2State_Init(void* pp) { Lzma2Dec_Init(&((CLzma2Dec_Spec*)pp)->decoder); }
...
static SRes Lzma2State_Code2(
  ...
  if (spec->outBufMode) {
    ...
    res = Lzma2Dec_DecodeToDic(
        &spec->decoder, dicLimit, src, srcLen, (ELzmaFinishMode)finishMode, &status2);
    *destLen = spec->decoder.decoder.dicPos - dicPos;
  } else
    res = Lzma2Dec_DecodeToBuf(
        &spec->decoder, dest, destLen, src, srcLen, (ELzmaFinishMode)finishMode, &status2);
...
static SRes Lzma2State_SetFromMethod(
    IStateCoder* p, Byte* outBuf, size_t outBufSize, ISzAllocPtr alloc) {
  CLzma2Dec_Spec* spec = (CLzma2Dec_Spec*)p->p;
  if (!spec) {
    spec = (CLzma2Dec_Spec*)ISzAlloc_Alloc(alloc, sizeof(CLzma2Dec_Spec));
    if (!spec) return SZ_ERROR_MEM;
    p->p = spec;
...
static SRes Lzma2State_ResetOutBuf(IStateCoder* p, Byte* outBuf, size_t outBufSize) {
  CLzma2Dec_Spec* spec = (CLzma2Dec_Spec*)p->p;
  if ((spec->outBufMode && !outBuf) || (!spec->outBufMode && outBuf)) return SZ_ERROR_FAIL;
  if (outBuf) {
    spec->decoder.decoder.dic = outBuf;
    spec->decoder.decoder.dicBufSize = outBufSize;
  }
...
static void MixCoder_Construct(CMixCoder* p, ISzAllocPtr alloc) {
  ...
  for (i = 0; i < MIXCODER_NUM_FILTERS_MAX; i++) p->coders[i].p = NULL;
}
...
static SRes MixCoder_SetFromMethod(
    CMixCoder* p, unsigned coderIndex, UInt64 methodId, Byte* outBuf, size_t outBufSize) {
  ...
  if (coderIndex == 0) return SZ_ERROR_UNSUPPORTED;
  return BraState_SetFromMethod(sc, methodId, 0, p->alloc);
}

static SRes MixCoder_ResetFromMethod(
    CMixCoder* p, unsigned coderIndex, UInt64 methodId, Byte* outBuf, size_t outBufSize) {
  IStateCoder* sc = &p->coders[coderIndex];
  switch (methodId) {
...
static SRes MixCoder_Code(
    CMixCoder* p, Byte* dest, SizeT* destLen, int destFinish, const Byte* src, SizeT* srcLen,
  ...
  *srcLen = 0;
  ...
  if (p->outBuf) {
...

    {
      ...
      res = coder->Code2(
          coder->p, NULL, &destLen2, src, &srcLen2, srcWasFinished, finishMode,
          // &wasFinished,
          &p->status);
    ...
    *srcLen = srcLen2;
    ...
    if (res != SZ_OK || srcWasFinished || wasFinished) p->wasFinished = True;
    ...
    if (p->numCoders == 1)
      *destLen = destLen2;
      ...
      for (i = 1; i < p->numCoders; i++) {
        ...
        if (wasFinished || (destFinish && p->outWritten == destLenOrig)) processed = p->outWritten;
    ...
    */
    ...
    for (i = 0; i < p->numCoders; i++) {
      ...
      if (p->results[i] != SZ_OK) {
        if (resMain == SZ_OK) resMain = p->results[i];
        continue;
      }

      res = coder->Code2(
          coder->p, dest2, &destLen2, src2, &srcLen2, srcFinished2, finishMode,
          // &encodingWasFinished,
          &status2);

      if (res != SZ_OK) {
        p->results[i] = res;
        if (resMain == SZ_OK) resMain = res;
      }
      ...
      if (!encodingWasFinished) {
        allFinished = False;
        if (p->numCoders == 1 && res == SZ_OK) p->status = status2;
      }
    ...
    if (!processed) {
      if (allFinished) p->status = CODER_STATUS_FINISHED_WITH_MARK;
      return resMain;
    }
  }
}
...
static BoolInt XzBlock_AreSupportedFilters(const CXzBlock* p) {
  ...
  unsigned i;
  {
    ...
    if (f->id != XZ_ID_LZMA2 || f->propsSize != 1 || f->props[0] > 40) return False;
  }
  ...
  for (i = 0; i < numFilters; i++) {
    ...
    if (f->id == XZ_ID_Delta) {
      if (f->propsSize != 1) return False;
        ...
        f->id < XZ_ID_Delta || f->id > XZ_ID_SPARC || (f->propsSize != 0 && f->propsSize != 4))
      return False;
  }
  return True;
}
...
SRes XzBlock_Parse(CXzBlock* p, const Byte* header) {
  ...
  if (CrcCalc(header, headerSize) != GetUi32(header + headerSize)) return SZ_ERROR_ARCHIVE;
  ...
  if (XzBlock_HasPackSize(p)) {
    ...
    if (p->packSize == 0 || p->packSize + headerSize >= (UInt64)1 << 63) return SZ_ERROR_ARCHIVE;
  }
  ...
  for (i = 0; i < numFilters; i++) {
    ...
    if (size > headerSize - pos || size > XZ_FILTER_PROPS_SIZE_MAX) return SZ_ERROR_ARCHIVE;
    ...
    {
      unsigned i;
      for (i = 0; i < size; i++) printf(" %2X", filter->props[i]);
    }
#endif
  }

  if (XzBlock_HasUnsupportedFlags(p)) return SZ_ERROR_UNSUPPORTED;

  while (pos < headerSize)
    if (header[pos++] != 0) return SZ_ERROR_ARCHIVE;
  return SZ_OK;
}
...
static SRes XzDecMix_Init(CMixCoder* p, const CXzBlock* block, Byte* outBuf, size_t outBufSize) {
  ...
  if (needReInit) {
    ...
    for (i = 0; i < numFilters; i++) {
      RINOK(
          MixCoder_SetFromMethod(p, i, block->filters[numFilters - 1 - i].id, outBuf, outBufSize));
    }
...
SRes XzUnpacker_Code(
    CXzUnpacker* p, Byte* dest, SizeT* destLen, const Byte* src, SizeT* srcLen, int srcFinished,
  ...
  for (;;) {
    ...
    if (p->state == XZ_STATE_BLOCK) {
      ...
      if (p->block.packSize != (UInt64)(Int64)-1) {
        ...
        if (rem == 0 && p->block.unpackSize == p->unpackSize) return SZ_ERROR_DATA;
      }
      ...
      */
      ...
      {
        res = MixCoder_Code(
            &p->decoder, (p->outBuf ? NULL : dest), &destLen2, destFinish, src, &srcLen2,
            srcFinished2, finishMode2);

        *status = p->decoder.status;
        XzCheck_Update(&p->check, (p->outBuf ? p->outBuf + p->outDataWritten : dest), destLen2);
        if (!p->outBuf) dest += destLen2;
    ...
    switch (p->state) {
      case XZ_STATE_STREAM_HEADER: {
        if (p->pos < XZ_STREAM_HEADER_SIZE) {
          if (p->pos < XZ_SIG_SIZE && *src != XZ_SIG[p->pos]) return SZ_ERROR_NO_ARCHIVE;
          if (p->decodeToStreamSignature) return SZ_OK;
          p->buf[p->pos++] = *src++;
          (*srcLen)++;
        } else {
      ...
      case XZ_STATE_BLOCK_HEADER: {
        if (p->pos == 0) {
          ...
          (*srcLen)++;
          if (p->buf[0] == 0) {
            if (p->decodeOnlyOneBlock) return SZ_ERROR_DATA;
        ...
        if (p->pos != p->blockHeaderSize) {
          ...
          if (cur > srcRem) cur = (UInt32)srcRem;
          ...
          if (!XzBlock_AreSupportedFilters(&p->block)) return SZ_ERROR_UNSUPPORTED;
          p->numTotalBlocks++;
      ...
      case XZ_STATE_STREAM_INDEX: {
        if (p->pos < p->indexPreSize) {
          (*srcLen)++;
          if (*src++ != p->buf[p->pos++]) return SZ_ERROR_CRC;
        } else {
          if (p->indexPos < p->indexSize) {
            UInt64 cur = p->indexSize - p->indexPos;
            if (srcRem > cur) srcRem = (SizeT)cur;
            ...
            if (memcmp(digest, p->shaDigest, SHA256_DIGEST_SIZE) != 0) return SZ_ERROR_CRC;
          }
        }
        break;
      }
      ...
      case XZ_STATE_STREAM_INDEX_CRC: {
        if (p->pos < 4) {
          ...
          p->pos = 0;
          if (CRC_GET_DIGEST(p->crc) != GetUi32(p->buf)) return SZ_ERROR_CRC;
        }
        break;
      }
      ...
      case XZ_STATE_STREAM_FOOTER: {
        ...
        if (p->pos == XZ_STREAM_FOOTER_SIZE) {
          ...
          p->padSize = 0;
          if (!Xz_CheckFooter(p->streamFlags, p->indexSize, p->buf)) return SZ_ERROR_CRC;
        }
        break;
      }
      ...
      case XZ_STATE_STREAM_PADDING: {
        if (*src != 0) {
          if (((UInt32)p->padSize & 3) != 0) return SZ_ERROR_NO_ARCHIVE;
          p->pos = 0;
          p->state = XZ_STATE_STREAM_HEADER;
        } else {
          (*srcLen)++;
...
SRes XzUnpacker_CodeFull(
    CXzUnpacker* p, Byte* dest, SizeT* destLen, const Byte* src, SizeT* srcLen,
    ECoderFinishMode finishMode, ECoderStatus* status) {
  XzUnpacker_Init(p);
  XzUnpacker_SetOutBuf(p, dest, *destLen);

  return XzUnpacker_Code(p, NULL, destLen, src, srcLen, True, finishMode, status);
}
...
static void XzDecMt_Callback_Parse(void* obj, unsigned coderIndex, CMtDecCallbackInfo* cc) {
  ...
  while (!coder->skipMode) {
    ...
    res = XzUnpacker_Code(
        &coder->dec, NULL, &destSize, cc->src, &srcSize2, cc->srcFinished, CODER_FINISH_END,
        &status);
    ...
    {
      if (!cc->srcFinished) return;  // SZ_OK;
      cc->state = coder->parseState = MTDEC_PARSE_END;
      return;  // SZ_OK;
    }
    cc->state = MTDEC_PARSE_OVERFLOW;
    ...
    if (coder->inPreSize == coder->blockPackTotal) {
      if (srcSize == 0) {
...

        {
          ...
          if (blockMax < coder->outPreSize) blockMax = coder->outPreSize;
          {
            UInt64 required = (UInt64)blockMax * (me->mtc.numStartedThreads + 1) * 2;
            if (me->props.memUseMax < required) cc->canCreateNewThread = False;
          }
        }
...
static SRes XzDecMt_Callback_PreCode(void* pp, unsigned coderIndex) {
  ...
  if (!coder->dec.headerParsedOk) return SZ_OK;
  ...
  if (!dest || coder->outBufSize < coder->outPreSize) {
    ...
    if (coder->outBufSize > me->unpackBlockMaxSize) me->unpackBlockMaxSize = coder->outBufSize;
  }
  ...
  {
    SRes res =
        XzDecMix_Init(&coder->dec.decoder, &coder->dec.block, coder->outBuf, coder->outBufSize);
    // res = SZ_ERROR_UNSUPPORTED; // to test
    coder->codeRes = res;
    if (res != SZ_OK) {
      // if (res == SZ_ERROR_MEM) return res;
      if (me->props.ignoreErrors && res != SZ_ERROR_MEM) return S_OK;
      return res;
    }
  }
...
static SRes XzDecMt_Callback_Code(
  ...
  *stop = True;
  ...
  if (coder->codeRes == SZ_OK) {
    ...
    // PRF(printf(" res = %d, srcSize2 = %d, outSizeCur = %d", res, (unsigned)srcProcessed,
    // (unsigned)outSizeCur));
...
static SRes XzDecMt_Callback_Write(
  ...
  *canRecode = True;
  ...
  if (!coder->dec.headerParsedOk || !coder->outBuf) {
    if (me->finishedDecoderIndex < 0) me->finishedDecoderIndex = coderIndex;
    return SZ_OK;
  }

  if (me->finishedDecoderIndex >= 0) return SZ_OK;
  ...
  *canRecode = False;

  {
    ...
    if (me->writeRes != SZ_OK) return me->writeRes;

    res = SZ_OK;
    {
      if (me->outSize_Defined) {
        const UInt64 rem = me->outSize - me->outProcessed;
        if (size > rem) size = (SizeT)rem;
      }

      for (;;) {
        size_t cur = size;
        size_t written;
        if (cur > XZDECMT_STREAM_WRITE_STEP) cur = XZDECMT_STREAM_WRITE_STEP;
  ...
  me->mtc.mtProgress.totalInSize = me->mtc.inProcessed;
  {
...

    {
      ...
      res = XzUnpacker_Code(
          dec, NULL, &outSizeCur, src, &srcProcessed,
      ...
      if (dec->state == XZ_STATE_STREAM_HEADER) {
        *needContinue = True;
        ...
        me->isBlockHeaderState_Write = False;
        {
          ...
          if (!crossBuf) return SZ_ERROR_MEM;
      ...
      if (status != CODER_STATUS_NEEDS_MORE_INPUT) { return E_FAIL; }

      if (me->mtc.readWasFinished) { return SZ_OK; }
    }
      ...
      for (;;) {
        ...
        if (inPos == inLim) {
          if (!me->mtc.readWasFinished) {
            ...
            if (inLim == 0 || me->mtc.readRes != SZ_OK) me->mtc.readWasFinished = True;
          }
        }
        ...
        res = XzUnpacker_Code(
            dec, NULL, &outProcessed, inData + inPos, &inProcessed,
...
static SRes XzDecMt_Decode_ST(
  ...
  if (!p->outBuf || p->outBufSize != p->props.outStep_ST) {
    ...
    if (!p->outBuf) return SZ_ERROR_MEM;
  ...
  for (;;) {
    ...
    if (p->outSize_Defined) {
      ...
      if (outSize >= rem) {
        outSize = (SizeT)rem;
        if (p->finishMode) finishMode = CODER_FINISH_END;
      }
    }
    ...
    res = XzUnpacker_Code(
        dec, p->outBuf + outPos, &outProcessed, inData + inPos, &inProcessed,
...
static SRes XzStatInfo_SetStat(
    const CXzUnpacker* dec, int finishMode, UInt64 readProcessed, UInt64 inProcessed, SRes res,
    ECoderStatus status, BoolInt decodingTruncated, CXzStatInfo* stat) {
  UInt64 extraSize;
  ...
  if (res == SZ_OK) {
    if (status == CODER_STATUS_NEEDS_MORE_INPUT) {
      ...
      extraSize = 0;
      if (!XzUnpacker_IsStreamWasFinished(dec)) res = SZ_ERROR_INPUT_EOF;
...
SRes XzDecMt_Decode(
    CXzDecMtHandle pp, const CXzDecMtProps* props, const UInt64* outDataSize, int finishMode,
    ISeqOutStream* outStream,
    ...
    *isMT = True;
...

    {
      ...
      if (!needContinue) {
        ...
        XzStatInfo_SetStat(
            dec, p->finishMode, p->mtc.readProcessed, p->mtc.inProcessed, codeRes, status,
            truncated, stat);
        ...
        if (stat->CombinedRes_Type == SZ_OK) stat->CombinedRes_Type = res;
        return res;
      }
  ...
  *isMT = False;

  {
    ...
    XzStatInfo_SetStat(
        &p->dec, p->finishMode, p->readProcessed, p->inProcessed, p->codeRes, p->status,
        False,  // truncated
        stat);
