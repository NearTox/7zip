...
#define XzBlock_HasUnsupportedFlags(p) \
  (((p)->flags & ~(XZ_BF_NUM_FILTERS_MASK | XZ_BF_PACK_SIZE | XZ_BF_UNPACK_SIZE)) != 0)
...
SRes Xzs_ReadBackward(
    CXzs* p, ILookInStream* inStream, Int64* startOffset, ICompressProgress* progress,
    ISzAllocPtr alloc);
...
typedef struct _IStateCoder {
  ...
  void (*Init)(void* p);
  SRes (*Code2)(
      void* p, Byte* dest, SizeT* destLen, const Byte* src, SizeT* srcLen, int srcWasFinished,
  ...
  SZ_OK
    status:
      ...
      CODER_STATUS_NEEDS_MORE_INPUT - maybe there are more xz streams,
                                      call XzUnpacker_IsStreamWasFinished to check that current
stream was finished SZ_ERROR_MEM  - Memory allocation error SZ_ERROR_DATA - Data error
