        ...
        if (distance >= kStartPosModelIndex) {
          ...
          if (posSlot < kEndPosModelIndex) {
            ...
            prob = probs + SpecPos;
            {
              UInt32 m = 1;
              distance++;
              do { REV_BIT_VAR(prob, distance, m); } while (--numDirectBits);
              distance -= m;
            }
          } else {
...
static int MY_FAST_CALL LzmaDec_DecodeReal2(CLzmaDec* p, SizeT limit, const Byte* bufLimit) {
  do {
    ...
    if (p->checkDicSize == 0) {
      ...
      if (p->processedPos == 0)
        if (p->code >= kBadRepCode) return SZ_ERROR_DATA;
    }
      ...
      if (state < 4) {
        ...
        if (posSlot >= kStartPosModelIndex) {
          ...
          }
          {
            unsigned i = 1;
            unsigned m = 1;
            do { REV_BIT_CHECK(prob, i, m); } while (--numDirectBits);
          }
        }
      }
    }
  ...
  while (p->remainLen != kMatchSpecLenStart) {
    ...
    if (p->tempBufSize == 0) {
...

      {
        ...
        rem -= kkk;
        if (lookAhead < rem) return SZ_ERROR_FAIL; /* some internal error */
        lookAhead -= rem;
      }
      (*srcLen) += (SizeT)lookAhead;
      src += lookAhead;
