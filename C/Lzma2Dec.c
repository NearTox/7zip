...
SRes Lzma2Dec_DecodeToDic(
    CLzma2Dec* p, SizeT dicLimit, const Byte* src, SizeT* srcLen, ELzmaFinishMode finishMode,
    ELzmaStatus* status) {
  SizeT inSize = *srcLen;
  *srcLen = 0;
...
ELzma2ParseStatus Lzma2Dec_Parse(
    CLzma2Dec* p, SizeT outSize, const Byte* src, SizeT* srcLen, int checkFinishBlock) {
  SizeT inSize = *srcLen;
  *srcLen = 0;

  while (p->state != LZMA2_STATE_ERROR) {
    if (p->state == LZMA2_STATE_FINISHED) return LZMA_STATUS_FINISHED_WITH_MARK;

    if (outSize == 0 && !checkFinishBlock) return LZMA_STATUS_NOT_FINISHED;

    if (p->state != LZMA2_STATE_DATA && p->state != LZMA2_STATE_DATA_CONT) {
      if (*srcLen == inSize) return LZMA_STATUS_NEEDS_MORE_INPUT;
      (*srcLen)++;
...

    {
      ...
      if (LZMA2_IS_UNCOMPRESSED_STATE(p)) {
        if (inCur == 0) return LZMA_STATUS_NEEDS_MORE_INPUT;
        ...
        *srcLen += inCur;
        ...
        if (inCur == 0) {
          if (p->packSize != 0) return LZMA_STATUS_NEEDS_MORE_INPUT;
...
SRes Lzma2Dec_DecodeToBuf(
    CLzma2Dec* p, Byte* dest, SizeT* destLen, const Byte* src, SizeT* srcLen,
...
SRes Lzma2Decode(
    Byte* dest, SizeT* destLen, const Byte* src, SizeT* srcLen, Byte prop,
