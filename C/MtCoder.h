...
/*
  ...
  if (not defined MTCODER__USE_WRITE_THREAD) : any coder thread can write data blocks to output
  stream
*/
...
typedef struct {
  SRes (*Code)(
      void* p, unsigned coderIndex, unsigned outBufIndex, const Byte* src, size_t srcSize,
      int finished);
